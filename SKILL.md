...
description: "Use when handling Clink wallet init/status/config, card or risk readiness, direct/UCP payment, refund, VIC/3DS events, finding a described product through Clink catalog search, listing tippable skills (支持打赏哪些 skill), tipping one or multiple skills, or installing a public skill by publisher/name with optional version or a Number from recent context."
metadata:
  version: "1.10.1"
  requires:
    node: ">=20"
...
Read multiple references when a workflow crosses boundaries. Example: a product order through UCP checkout needs `clink-cli-invocation.md`, `clink-wallet-config.md`, `clink-instruction.md`, `clink-ucp-checkout.md`, and sometimes `clink-async-events.md`. A described product that must first be found needs `clink-catalog-discovery.md` before those. Any workflow that ends up emitting a URL for the user — binding, Passkey, 3DS, risk — also needs `clink-browser-handoff.md`.
...
| --- | --- | --- |
| OAuth device verification (`wallet init` live stderr) | `USER_DEVICE_ONLY`, single load | none; the original init process resolves |
...
Pass `--no-open` on every link-producing command, not only `wallet init`, and check `defaultOpenLinks` once per workflow with `clink config get --format json` — it lives in the machine-wide config every build shares, so a stored `true` re-arms host-side auto-open. Completion is always proven by the event, never by the browser, which is what makes these flows work no matter which browser or device the user chose. Never add browser-side verification that a page opened.
...
| `SHOW_OAUTH_VERIFICATION_URL_AND_WAIT` | Read the exact verification URL only from the original `wallet init --no-open` process's live stderr, then output the complete value of `authorizationUrl` verbatim on its own line. Do not add, remove, parse, encode, decode, rebuild, reduce to an origin, or truncate any character; preserve the query after `?` and the fragment after `#`. Send it once, keep that same process running, and ask the user to complete authorization in the browser. Never open the URL from the Agent runtime or start another init process; this is a `USER_DEVICE_ONLY` single-load page, so every channel listed under `HANDOFF_TO_USER_DEVICE` is barred and a second load can re-trigger verification-code sending. |
...
| The original `wallet init --no-open` process's live stderr prints `Complete authorization in your browser:` | Surface the following URL immediately and exactly once, keep that same process alive, and wait for browser authorization; never open it from the Agent runtime or ask the user to send an email OTP to the agent. |
...
| Any URL a person must act on is about to be emitted | Classify it with `classifyPageHandoff` first, pass `--no-open` on the command that produced it, and hand it off per its actor. Card, Passkey, 3DS, OAuth, and instruction update/cancel pages are `USER_DEVICE_ONLY`; do not let the agent runtime open, navigate, preview, or screenshot them by any channel. |
...
- Suppress browser launch on every link-producing command, not only `wallet init`: pass `--no-open` to `card binding-link`, `card setup-link`, `card modify-link`, `risk link`, `instruction create`, `instruction sign-url`, `instruction update`, and `instruction cancel` as well. `--no-open` disables launch only and never disables the built-in watch. Verify `defaultOpenLinks` once per workflow with `clink config get --format json`; it lives in the machine-wide config every build shares, so an earlier stored `true` re-arms host-side auto-open — which in a container, sandbox, or CI opens the page where the user cannot see it.
- Emit `OAUTH_DEVICE_VERIFICATION`, card, and 3DS URLs exactly once. They are single-load pages: re-sending as a nudge can invalidate a one-time token or re-trigger verification-code sending.
...
- `--line-items` carries `item.price` as a **major-unit decimal string** such as `"26.00"` for both checkout create and update. `ucp-checkout create` scales it by `--currency`; live update reads the checkout currency and scales decimal strings before PUT, while update `--dry-run` requires `--currency`. Catalog candidates and `parse-item` report `unitPriceMinor` / `price.amount` in **minor units** such as `2600`; putting that minor-unit value into a decimal-string field can bill 100x the agreed amount. Convert once before building `--line-items`; JSON integer money values are accepted only as legacy minor-unit update inputs. `totalAmountMinor` stays in minor units for authorization matching.
...
| Initialize wallet | `clink wallet init --email <email> --no-open --format json`; the name comes from the email text before `@` and there is no `--name` flag, then read the verification URL once from live stderr and keep the original process running while the user completes browser authorization |
