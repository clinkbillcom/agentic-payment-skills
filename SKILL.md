...
| `VERIFY_AUTHORIZATION_AFTER_WATCH_GAP` | The watch ended without this instruction's activation — it hit the 15-minute timeout, the runtime killed the foreground command, or only an unrelated instruction's event arrived. None of these mean authorization failed, so never report failure here. Run `clink-cli instruction get --purchase-instruction-id <id> --format json` first; surface a failed GET instead of treating it as pending. Only when the GET succeeds with a non-`ACTIVE` status and the user may still be authorizing should you restart the returned `pollCommand`, rather than re-creating the draft or re-sending the URL. |
...
| `REFRESH_PAYMENT_INSTRUMENT` | Run `clink-cli card binding-link --no-watch --format json` and resolve the selected/default instrument before the authorization capability gate. |
...
| `GET_REST_ENDPOINT` | The profile returned parseable JSON. Run `clink-cli tool get-rest-endpoint --url <standard_ucp_url> --format json` and reclassify; `provider=clinkbill` selects internal checkout and any other provider selects external. |
...
| `POLL_PAYMENT_SUCCESS_EVENT` | Immediately run `clink-cli events poll --type agent_order.succeeded --max-wait 900 --format json` after UCP checkout complete returns `completed`; correlate the event by `checkoutId` (primary anchor). Pass `omsOrderId` from the complete response into `expectedResource` so the next step can fetch the order. |
| `FETCH_OMS_ORDER` | The payment success event matched and `omsOrderId` is known. Run the returned `orderCommand` (`clink-cli ucp-order get --order-id <omsOrderId> --format json`) to fetch the OMS order, then surface both the payment event and the order to the caller. If `ucp-order get` fails, the payment is still confirmed — surface the success event and report the order fetch error separately; never reclassify a confirmed payment as failed. |
...
| `RUN_DIRECT_PAY_WORKFLOW` | A known merchant with no product intent. Classify fulfillment, refresh instruments, and run `classifyPaymentAuthorizationResolver` before any `clink-cli pay`. |
...
| `GET_MERCHANT_LIST` | Run `clink-cli tool internal-ucp get-merchant-list --format json` before any catalog search; the environment lock selects the returned list. |
...
- Catalog product discovery must load `clink-cli tool internal-ucp get-merchant-list --format json` before the first search, and may match intent only against a candidate's `description`. Candidates require a `merchant_id`, a non-empty `description`, and must not be `enabled:false`. A matched `merchantId` outside the loaded list is rejected, never searched.
- Catalog search paths are not interchangeable. `clink-cli ucp-catalog search` is merchant-scoped and always needs `--merchant-id`; `clink-cli catalog search` is the cross-merchant path and never takes `--merchant-id`. An empty scoped result widens to broad search, but a CLI/API error surfaces and stops instead of widening past a real failure.
...
- After UCP checkout complete returns `completed`, immediately run `clink-cli events poll --type agent_order.succeeded --max-wait 900 --format json`, correlate the returned event to the current checkout/order/session when identifiers are available, then fetch the order details with `clink-cli ucp-order get --order-id <omsOrderId> --format json` when the complete response carried the OMS order id, and send the matched success event/message plus the order back to the caller.
...
| Wait for optional Agent Pay/Tip account evidence | `clink-cli events poll --type account-created,account-reloaded --max-wait 60 --format json`; classify the same result for both types |
| Initialize wallet | `clink-cli wallet init --email <email> --no-open --format json`; the name comes from the email text before `@` and there is no `--name` flag, then read the verification URL once from live stderr and keep the original process running while the user completes browser authorization |
...
| Bind first card and wait | `clink-cli card binding-link --format json`; omit `--no-watch` and keep the process alive until the second envelope carries `payment_method.added` |
| Add another payment method | `clink-cli card setup-link --format json`; emit the returned URL, then keep this same process alive until the second envelope carries `payment_method.added`. Omitting `--no-watch` is what starts the watch; do not also run `events poll` |
| Switch default card or manage existing cards | `clink-cli card modify-link --format json`; there is no CLI command that sets a default card directly, so emit this management URL and let the user change it in the browser. Keep the process alive until the second envelope carries `payment_method.updated` or `payment_method.default_change` |
| Get risk-rule config URL and wait | `clink-cli risk link --format json`; keep the process alive until the second envelope carries `risk_rule.updated` |
...
| Wait for instruction activation | Keep the `instruction create`/`sign-url` process alive and read its second envelope. Use `clink-cli events poll --type purchase_instruction.activated --no-ack --format json` only to resume after that watch timed out or was killed |
...
| Create VIC instruction draft | `clink-cli instruction create ... --format json`; omit `--no-watch` and keep the process alive — it prints the draft, then blocks watching for `purchase_instruction.activated` and prints a second envelope |
| Print instruction Passkey URL | `clink-cli instruction sign-url ... --format json`; omit `--no-watch` and keep the process alive for the same activation watch |
| Get one VIC instruction | `clink-cli instruction get --purchase-instruction-id <id> --format json` |
| List catalog-supported merchants | `clink-cli tool internal-ucp get-merchant-list --format json` |
| Search one merchant's catalog | `clink-cli ucp-catalog search --merchant-id <id> --query <text> --format json` |
| Search across merchants and platform stores | `clink-cli catalog search --query <text> --format json` |
| Search eats365 in HK, optionally targeting one known store in the returned groups | `clink-cli catalog search --query <text> --channel-type eats365 --context '{"address_country":"HK"}' --format json`; when a `storeId` is established, locally keep only groups with that exact `store_id` and recompute the count |
...
| Wait for UCP payment success | `clink-cli events poll --type agent_order.succeeded --max-wait 900 --format json` |
...
- Passing `--merchant-id` to `clink catalog search`, or omitting it from `clink-cli ucp-catalog search`.
