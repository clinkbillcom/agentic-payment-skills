...
| `SHOW_OAUTH_VERIFICATION_URL_AND_WAIT` | Send the exact verification URL once, keep the same `wallet init` process running, and ask the user to complete authorization in the browser. Automatic browser-open failure is a warning, not a terminal failure. |
...
| `wallet init` prints `Complete authorization in your browser:` | Surface the following URL immediately, keep the process alive, and wait for browser authorization; never ask the user to send an email OTP to the agent. |
...
- Wallet initialization uses OAuth Device Authorization. Stream stderr, send the verification URL once, keep the original process running, and wait for the user to finish browser authorization. Never ask the user to send an email OTP, never add `--otp`, and never treat automatic browser-open failure as authorization failure.
...
| Initialize wallet | `clink-cli wallet init --email <email> --name <name> --format json` and keep the process running while the user completes browser authorization |
