...
- Wallet initialization uses OAuth Device Authorization. Always invoke `wallet init` with `--open`. When it reports `Opening your browser...`, say only that the CLI requested the system browser; do not claim a visible window was confirmed, and keep the original process waiting without repeating the URL. Only after a browser-launch failure may the Agent send the validated URL from that same process once. Never navigate, preview, or prefetch the URL with an Agent browser, start another init process, ask the user to send an email OTP, or add `--otp`.
...
- Emit card and 3DS URLs exactly once. For `OAUTH_DEVICE_VERIFICATION`, keep the URL hidden after the system-browser request and emit it once only after a reported launch failure. These are single-load pages: re-sending as a nudge can invalidate a one-time token or re-trigger verification-code sending.
