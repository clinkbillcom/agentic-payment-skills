...
- For Shopify product URLs, resolve variant IDs through `classifyUcpItemIdResolution`: direct variant links use the `variant` query parameter; SPU slug links fetch `<product_url>.js`, parse the variants array, and select by explicit user choice. If multiple variants remain, ask instead of guessing; non-Shopify or unresolved URLs fall back to `clink-cli tool item-id --url`.
