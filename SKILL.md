...
| Restricted-category screening before any `instruction create`, including scheduled pre-authorization drafts | `references/clink-restricted-categories.md` |
...
| `card binding-link` / `setup-link` / `modify-link` | `USER_DEVICE_ONLY` | `payment_method.added` / `.updated` / `.default_change` |
...
Maintain an **environment lock** from the effective `wallet status` base URL. The environment is chosen once by `wallet init` and saved for every later command (see `references/clink-cli-invocation.md`). This distribution pins `wallet init` to production, so `--sandbox`/`--test` exit 2 here; other distributions pin sandbox/UAT or test the same way. Every command other than `wallet init` rejects those flags, and `--base-url` no longer exists. Do not change the saved environment or `CLINK_BASE_URL` mid-workflow. Individual command recipes stay environment-neutral.
...
| `WAIT_FOR_WALLET_INIT_PROGRESS` | Keep the original `wallet init --open` process running without showing its URL or claiming the browser opened until stderr reports either the browser-open request or its failure. |
| `TELL_USER_BROWSER_OPEN_REQUESTED_AND_WAIT` | For `wallet init --open`, do not show the verification URL or claim that a visible window was confirmed. Tell the user the CLI requested the system browser, ask them to finish email verification and click Confirm in the resulting window, and keep the same process waiting. |
| `SHOW_OAUTH_VERIFICATION_URL_AND_WAIT` | Use only when `--open` reports browser-launch failure. Read the exact verification URL only from the current process's latest wallet-init attempt segment, then output the complete value of `authorizationUrl` verbatim on its own line. Do not add, remove, parse, encode, decode, rebuild, reduce to an origin, or truncate any character; preserve the query after `?` and the fragment after `#`. Send it once and keep that same process running. |
| `RETURN_WALLET_PLAN` | Report a `--dry-run` Device Authorization request as planned, not initialized. |
| `RETURN_WALLET_READY` | For `wallet init`, require `hasAuthorization=true`, `authorizationType=oauth`, `hasCustomerApiKey=false`, and a non-empty `customerId`; init no longer echoes `oauthRequired`. For `wallet status`, require the complete effective-credential markers documented in `references/clink-wallet-config.md`. Do not force a never-OAuth CSK user to migrate during another operation. When `data.bindingUrl` is non-empty, proactively send its origin-only card-binding URL; never expose secrets. |
| `REFRESH_PAYMENT_INSTRUMENT_LIST` | Run `clink card binding-link --no-watch --format json` and resolve the selected/default payment instrument before direct/session pay. |
...
| `CONTINUE_INSTRUCTION_CREATION` | Restricted-category screening found no match for the complete purchase context. Proceed with the pending `instruction create`. |
...
| `WAIT_AUTHORIZATION_ACTIVATION` | A successful verification still reports `CREATED`, `PENDING`, or `INPROGRESS`, so activation may still arrive. Keep the pending payment intent frozen and keep waiting; do not re-create the draft, re-send the Passkey URL, or start a second watcher. `COMPLETED`, `CANCELLED`, `EXPIRED`, `DECLINED`, missing, and unknown statuses are errors, not pending. |
| `VERIFY_RESOURCE_STATUS` | After `classifyEventPollObservation` correlates the event by `instructionId` / `purchaseInstructionId`, run the waitSpec verify command, normally `clink instruction get --purchase-instruction-id <id> --format json`. |
...
| `VERIFY_CHECKOUT_BEFORE_RETRY` | Checkout state is unknown. Verify through `ucp-checkout get` before any retry; never blind-retry a checkout that may have charged. |
...
| `POLL_PAYMENT_SUCCESS_EVENT` | Immediately run `clink events poll --type agent_order.succeeded --max-wait 900 --format json` after UCP checkout complete returns `completed`; correlate the event by `checkoutId` (primary anchor). Pass `omsOrderId` from the complete response into `expectedResource` so the next step can fetch the order. |
| `FETCH_OMS_ORDER` | The payment success event matched and `omsOrderId` is known. Run the returned `orderCommand` (`clink ucp-order get --order-id <omsOrderId> --format json`) to fetch the OMS order, then surface both the payment event and the order to the caller. If `ucp-order get` fails, the payment is still confirmed — surface the success event and report the order fetch error separately; never reclassify a confirmed payment as failed. |
| `RETURN_PAYMENT_SUCCESS_EVENT` | Surface the matched `agent_order.succeeded` event/message to the caller when no `omsOrderId` is available; do not claim merchant fulfillment. |
...
| `RUN_CATALOG_DISCOVERY_WORKFLOW` | Run the catalog discovery FSM with the returned `catalogQuery`, then present the candidates and wait for the user to select one. Do not enter UCP checkout from a described product alone. |
...
| `GET_MERCHANT_LIST` | Run `clink tool internal-ucp get-merchant-list --format json` before any catalog search; the environment lock selects the returned list. |
...
| `RUN_MERCHANT_SCOPED_CATALOG_SEARCH` | Run `clink ucp-catalog search --merchant-id <id> --query <text> --format json` for the matched merchant. An empty `products` array widens to broad search; an error surfaces and stops. |
| `RUN_BROAD_CATALOG_SEARCH` | Run `clink catalog search --query <text> [--channel-type <channel>] [--context <json>] --format json` without `--merchant-id`. After the merchant-list preflight, an established channel/store target goes directly here because merchant-scoped search cannot apply either constraint. Put the channel in top-level `--channel-type`; pass `context.address_country` only for HK/SG. `--ext` is record-only and does not filter. The CLI has no store-search flag, so keep an established `storeId` as target identity, then filter returned groups by exact `store_id` and recompute the product count. |
...
| `wallet init --open` prints `Opening your browser...` without a browser-launch failure | Do not repeat the URL or claim a visible window was confirmed. Tell the user the CLI requested the system browser, ask them to finish email verification and click Confirm in the resulting window, and keep the same process waiting. |
| `wallet init --open` reports browser-launch failure | Surface the validated URL from the original process exactly once, keep that same process alive, and wait for browser authorization; do not ask the user to send an email OTP to the agent. |
...
| Any `clink instruction create` is about to run (direct/session no-match, UCP no-match, or scheduled pre-authorization draft) | Run `classifyInstructionRestriction` from `lib/restricted-categories.mjs` over the complete context described in `references/clink-restricted-categories.md`. Only `CONTINUE_INSTRUCTION_CREATION` may run the command; refuse a restricted category and fix incomplete/invalid gate input before retrying classification. |
...
| User describes a product to find, with no product link | Run `classifyCatalogDiscovery`. Load `clink tool internal-ucp get-merchant-list --format json`, match intent against merchant `description` values, then run `clink ucp-catalog search --merchant-id <id> --query <text>` for a match or `clink catalog search --query <text> [--channel-type <channel>] [--context <json>]` when nothing matched or the scoped search was empty. Return candidates without starting checkout. |
...
- Never run `clink instruction create` on any path — direct/session no-match, UCP no-match, or scheduled pre-authorization — until `classifyInstructionRestriction` answers `CONTINUE_INSTRUCTION_CREATION` for that exact purchase context. Follow `references/clink-restricted-categories.md` for the full category list, semantic assertions, invalid-input handling, and terminal refusal behavior.
...
- Catalog product discovery must load `clink tool internal-ucp get-merchant-list --format json` before the first search, and may match intent only against a candidate's `description`. Candidates require a `merchant_id`, a non-empty `description`, and must not be `enabled:false`. A matched `merchantId` outside the loaded list is rejected, never searched.
- Catalog search paths are not interchangeable. `clink ucp-catalog search` is merchant-scoped and always needs `--merchant-id`; `clink catalog search` is the cross-merchant path and never takes `--merchant-id`. An empty scoped result widens to broad search, but a CLI/API error surfaces and stops instead of widening past a real failure.
...
- Wallet initialization uses OAuth Device Authorization. Always invoke `wallet init` with `--open`. When it reports `Opening your browser...`, say only that the CLI requested the system browser; do not claim a visible window was confirmed, and keep the original process waiting without repeating the URL. Only after a browser-launch failure may the Agent send the validated URL from that same process once. Do not start another init merely to recover or repeat the URL for the same active attempt. An explicit `WALLET_RELOGIN`, or an expired or terminal attempt, starts exactly one fresh init and lets the CLI supersede the older attempt. Never navigate, preview, or prefetch the URL with an Agent browser, ask the user to send an email OTP, or add `--otp`.
...
- After UCP checkout complete returns `completed`, immediately run `clink events poll --type agent_order.succeeded --max-wait 900 --format json`, correlate the returned event to the current checkout/order/session when identifiers are available, then fetch the order details with `clink ucp-order get --order-id <omsOrderId> --format json` when the complete response carried the OMS order id, and send the matched success event/message plus the order back to the caller.
...
| Initialize wallet | `clink wallet init --email <email> --open --format json`; the name comes from the email text before `@` and there is no `--name` flag. When the CLI requests the system browser, tell the user to complete authorization in the resulting window without claiming it was visibly confirmed, and keep the original process running; show the URL only after browser-launch failure |
...
| Bind first card and wait | `clink card binding-link --no-open --format json`; omit `--no-watch` and keep the process alive until the second envelope carries `payment_method.added` |
...
| Switch default card or manage existing cards | `clink card modify-link --no-open --format json`; there is no CLI command that sets a default card directly, so emit this management URL and let the user change it in the browser. Keep the process alive until the second envelope carries `payment_method.updated` or `payment_method.default_change` |
...
| List catalog-supported merchants | `clink tool internal-ucp get-merchant-list --format json` |
| Search one merchant's catalog | `clink ucp-catalog search --merchant-id <id> --query <text> --format json` |
| Search across merchants and platform stores | `clink catalog search --query <text> --format json` |
| Search eats365 in HK, optionally targeting one known store in the returned groups | `clink catalog search --query <text> --channel-type eats365 --context '{"address_country":"HK"}' --format json`; when a `storeId` is established, locally keep only groups with that exact `store_id` and recompute the count |
| Analyze UCP product item(s) | `clink tool parse-item --url <item_url> --format json` |
| Search a merchant UCP catalog | `clink ucp-catalog search --merchant-id <id> --query <text> [--limit <n>] [--cursor <cursor>] --format json` |
| Get one UCP catalog product | `clink ucp-catalog product --merchant-id <id> --product-id <id> --format json` |
...
| Wait for UCP payment success | `clink events poll --type agent_order.succeeded --max-wait 900 --format json` |
...
- Running `clink instruction create` without screening the purchase first, or treating a clean keyword scan as clearance when the meaning is plainly restricted — assert the category instead of letting the backend reject the draft as illegal content.
