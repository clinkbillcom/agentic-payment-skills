...
| `RESOLVE_CHECKOUT_ROUTE` | Run `classifyUcpCheckoutRoute` after item selection. Route `www.bruceleeclub.com` or parseable `/.well-known/ucp-clink` JSON as a standard UCP candidate, then run the returned `clink-cli tool get-rest-endpoint --url <standard_ucp_url> --format json`. Route to standard UCP checkout only when provider is `clinkbill`; if provider is not `clinkbill` or endpoint discovery fails, route to external UCP checkout. |
...
| Discovered product order | Resolve or explore to one product detail URL, run `clink-cli tool parse-item --url <item_url> --format json`, select exactly one available item, let agent/FSM supply quantity, merchantCategoryCode, fulfillmentType, and shipping when required. If fulfillment is `UNKNOWN`, ask before checkout. If physical goods ship, collect a standard complete shipping address before instruction list or checkout create. Resolve paymentInstrumentId, list/match ACTIVE instruction+mandate only when Visa + VIC is ready, start the instruction workflow and stop until activation when no match exists, then run `classifyUcpCheckoutRoute`: `www.bruceleeclub.com` and parseable `https://domain/.well-known/ucp-clink` JSON are standard UCP candidates; candidates must run `clink-cli tool get-rest-endpoint --url <standard_ucp_url> --format json`, use standard UCP checkout only for `provider=clinkbill`, and use external UCP checkout when provider is not `clinkbill` or discovery fails. Create checkout, parse checkoutId, complete checkout, immediately poll `agent_order.succeeded`, and return the matched event/message; do not use plain `pay`. |
...
- UCP checkout product orders must resolve checkout route before checkout create with `lib/ucp-checkout-route-fsm.mjs`. Derive the canonical merchant domain from `parse-item` `merchantDomain`, `merchantOrigin`, or selected item URL. The known standard UCP domain allowlist currently contains `www.bruceleeclub.com`; non-allowlisted domains require the returned `/.well-known/ucp-clink` probe before external checkout. Allowlist hits and successful parseable JSON responses are standard UCP candidates, not final standard routes: run `clink-cli tool get-rest-endpoint --url <standard_ucp_url> --format json`, route to standard UCP checkout only when provider is `clinkbill`, pass `--endpoint <rest_endpoint>` when the clinkbill endpoint is non-empty, and route to external UCP checkout when provider is not `clinkbill`, endpoint discovery fails, or `standard_ucp_profile_absent` is returned.
...
| Resolve standard UCP REST endpoint | `clink-cli tool get-rest-endpoint --url <standard_ucp_url> --format json` |
