...
| Catalog product discovery from a described product, supported-merchant list, merchant intent matching, merchant-scoped or broad catalog search, channel/region/store narrowing | `references/clink-catalog-discovery.md` |
...
- find a product in Clink catalogs when the user describes it instead of supplying a link: load the supported merchant list, match intent against merchant descriptions, run a merchant-scoped or broad catalog search, narrow by channel/region/store when context establishes one, and hand discovery back to browser/MCP/Skill tools when the catalogs have no match
...
| `START_AUTHORIZATION_ACTIVATION_WATCH` | Send the Passkey URL and immediately start `clink-cli events poll --type purchase_instruction.activated --no-ack --format json` from the returned waitSpec; do not wait for the user to report completion before listening. `instruction create`/`sign-url` print `Watch not started (--no-watch)` plus the exact poll command on stderr — that line is the handoff, so run it before sending the URL rather than treating it as a notice. Asking the user to report back instead of listening leaves the activation unobserved. |
| `WAIT_AUTHORIZATION_ACTIVATION` | The activation event has not arrived yet. Keep the pending payment intent frozen and keep waiting; do not re-create the draft, re-send the Passkey URL, or start a second watcher. |
...
| `SURFACE_AUTHORIZATION_ERROR` | Report the authorization error and stop the current payment intent; do not resume payment, retry the draft, or claim the instruction is usable. An instruction-ID mismatch means a different instruction activated and this intent must not resume. |
...
| `RUN_BROAD_CATALOG_SEARCH` | Run `clink-cli catalog search --query <text> [--ext <json>] --format json` without `--merchant-id`. Include `--ext` only when context establishes `channel_type`, optionally with `region` and `store_id`. |
...
| `ASK_FOR_CATALOG_INPUT` | Ask for the missing or unsupported catalog input (query, `channel_type`, or a supported `region`) before running any search command. |
...
| User describes a product to find, with no product link | Run `classifyCatalogDiscovery`. Load `clink-cli tool internal-ucp get-merchant-list --format json`, match intent against merchant `description` values, then run `clink-cli ucp-catalog search --merchant-id <id> --query <text>` for a match or `clink-cli catalog search --query <text> [--ext <json>]` when nothing matched or the scoped search was empty. Return candidates without starting checkout. |
| Catalog discovery context establishes an `eats365` channel, region, or one store | Pass `--ext` with `channel_type` (always `eats365`; normalize `eat365`), plus `region` (`hk` only) and `store_id` when context supplies them. Never invent a store id, and never send `region`/`store_id` without `channel_type`. |
...
- When no-match Visa + VIC ready pay starts instruction creation, preserve a pending payment intent with its draft instruction, then wait for `purchase_instruction.activated` through `clink-cli events poll --type purchase_instruction.activated --no-ack --format json`. Resume through `RESUME_AUTHORIZED_PAYMENT` only after `classifyEventPollObservation` matches the activation by the same `instructionId` / `purchaseInstructionId` and `classifyAuthorizationActiveVerification` proves the instruction is ACTIVE through `instruction get --purchase-instruction-id <id> --format json`. Activation is authorization, not completion: the frozen intent must continue to checkout or pay in the same turn. A different instruction activation on the same card must not resume this intent. The merchant skill must not manually provide `instruction_id` or `mandate_id` or call pay outside this payment intent.
...
- Catalog `--ext` narrowing uses `channel_type` `eats365` (normalize the `eat365` spelling), `region` `hk` only, and a `store_id` taken from context or an earlier broad-search group. Never invent a store id or region, and never send `region` or `store_id` without `channel_type`; an unsupported region asks instead of searching.
...
- Never run `wallet init` as hidden recovery inside payment, checkout, tip, or refund execution. For exit code 3 or an OAuth `401`, stop the current operation and start an explicit wallet setup/reauthorization workflow after collecting only the missing email. Do not ask for a name by default: the CLI uses the email text before `@`; pass optional `--name` only when the user explicitly requests an override or the derived name is invalid or longer than 50 characters. For `403`, surface the permission error without refreshing or retrying.
...
- Async completion is event-driven. Never claim binding, refund, VIC registration, instruction activation, risk-rule update, or post-3DS order completion until the matching event has been observed through the built-in link watch or `events poll`. Start that listener at URL-emit time, concurrently and non-blocking, not after the user reports completion; when a flow has multiple valid readiness events (for example VIC registration), watch all of them and confirm against authoritative status (`card get`, `refund get`) rather than a single event type. Two mechanisms deliver this and they must not be combined on the same URL: link commands that watch by default (`card binding-link`, `card setup-link`, `card modify-link`, `risk link`, `instruction update`, `instruction cancel`, and `pay` returning a 3DS redirect) need only `--no-watch` omitted and their process kept alive, while `instruction create`, `instruction sign-url`, and a completed UCP checkout take exactly one correlated `events poll`. A polling loop the agent invents over a cache-reading command such as `card list` is never a substitute for either.
...
| Initialize wallet | `clink-cli wallet init --email <email> [--name <name>] --no-open --format json`; omit `--name` to use the email text before `@`, then read the verification URL once from live stderr and keep the original process running while the user completes browser authorization |
...
| Create VIC instruction draft | `clink-cli instruction create ... --no-watch --format json`, then run the Event FSM waitSpec |
...
| Search one eats365 HK store | `clink-cli catalog search --query <text> --ext '{"channel_type":"eats365","region":"hk","store_id":"<store_id>"}' --format json` |
...
- Sending `eat365` as the catalog channel type, or putting `region`/`store_id` in `--ext` without `channel_type`.
...
- Passing `--no-watch` without immediately starting the workflow's explicit Event FSM watcher, or omitting it when you only wanted a cache refresh. `instruction create/sign-url` intentionally use `--no-watch` plus one correlated Event FSM poll.
