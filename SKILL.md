...
| `RESOLVE_CHECKOUT_ROUTE` | Run `classifyUcpCheckoutRoute` after item selection. Route `www.bruceleeclub.com` through standard UCP checkout; route all other resolved domains through external UCP checkout. |
...
| Discovered product order | Resolve or explore to one product detail URL, run `clink-cli tool parse-item --url <item_url> --format json`, select exactly one available item, let agent/FSM supply quantity, merchantCategoryCode, fulfillmentType, and shipping when required, resolve authorization only when Visa + VIC is ready, then run `classifyUcpCheckoutRoute`: `www.bruceleeclub.com` uses standard UCP checkout and every other resolved domain uses external UCP checkout; do not use plain `pay` |
...
- UCP checkout product orders must resolve checkout route before checkout create with `lib/ucp-checkout-route-fsm.mjs`. Derive the canonical merchant domain from `parse-item` `merchantDomain`, `merchantOrigin`, or selected item URL. The known standard UCP domain allowlist currently contains `www.bruceleeclub.com`; otherwise route to external UCP checkout.
