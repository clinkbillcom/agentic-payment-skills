...
`--environment production` for this production distribution, including standalone login
before any recommendation. Preserve that environment
through all continuations. Do not inspect config or switch to sandbox/test to
...
```text
<Skill Path>/bin/visa-cli visa login --environment production --format json
```
...
```text
<Skill Path>/bin/visa-cli visa login --environment production \
...
```text
<Skill Path>/bin/visa-cli visa payment-method resolve \
  --environment production --payment-instrument-id <id> \
...
For explicit login-only requests, use Step2 `visa login --environment production`
...
- Use `visa login --environment production --format json` for Visa Skill login
