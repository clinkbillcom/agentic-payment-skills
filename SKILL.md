...
Read multiple references when a workflow crosses boundaries. Example: a product order through UCP checkout needs `clink-cli-invocation.md`, `clink-wallet-config.md`, `clink-instruction.md`, `clink-ucp-checkout.md`, and sometimes `clink-async-events.md`. A described product that must first be found needs `clink-catalog-discovery.md` before those.
...
| Refresh payment-instrument list without waiting | `clink card binding-link --no-watch --format json` (returns `paymentMethodsVoList` and updates the local cache) |
| Bind first card and wait | `clink card binding-link --format json`; omit `--no-watch` and keep the process alive until the second envelope carries `payment_method.added` |
| Add another payment method | `clink card setup-link --format json`; emit the returned URL, then keep this same process alive until the second envelope carries `payment_method.added`. Omitting `--no-watch` is what starts the watch; do not also run `events poll` |
| Switch default card or manage existing cards | `clink card modify-link --format json`; there is no CLI command that sets a default card directly, so emit this management URL and let the user change it in the browser. Keep the process alive until the second envelope carries `payment_method.updated` or `payment_method.default_change` |
| Get risk-rule config URL and wait | `clink risk link --format json`; keep the process alive until the second envelope carries `risk_rule.updated` |
...
| Create VIC instruction draft | `clink instruction create ... --format json`; omit `--no-watch` and keep the process alive — it prints the draft, then blocks watching for `purchase_instruction.activated` and prints a second envelope |
| Print instruction Passkey URL | `clink instruction sign-url ... --format json`; omit `--no-watch` and keep the process alive for the same activation watch |
