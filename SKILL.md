...
This skill executes wallet, card, payment, refund, risk, VIC instruction, event, and local config commands. It does not decide pricing, entitlement, or merchant receipt confirmation.
...
Read multiple references when a workflow crosses boundaries. Example: a Visa booking needs `clink-cli-invocation.md`, `clink-wallet-profile.md`, `clink-instruction.md`, and `clink-async-events.md`.
...
When another skill needs to charge the user:
