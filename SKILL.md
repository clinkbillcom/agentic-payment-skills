...
| `wallet init --open` prints `Opening your browser...` without a browser-launch failure | Do not repeat the URL. Tell the user the Clink authorization page opened, ask them to finish email verification and click Confirm, and keep the same process waiting. |
...
- Wallet initialization uses OAuth Device Authorization. Always invoke `wallet init` with `--open`. When it reports `Opening your browser...`, tell the user the Clink authorization page opened and keep the original process waiting without repeating the URL. Only after a browser-launch failure may the Agent send the validated URL from that same process once. Never navigate, preview, or prefetch the URL with an Agent browser, start another init process, ask the user to send an email OTP, or add `--otp`.
...
| Initialize wallet | `clink wallet init --email <email> --open --format json`; the name comes from the email text before `@` and there is no `--name` flag. When the CLI opens the system browser, tell the user to complete authorization there and keep the original process running; show the URL only after browser-launch failure |
