...
- Anonymous supported-merchant discovery with `clink ucp-merchant list --internal`: it calls the selected environment's `GET /agent/ucp/merchants`, uses the server-filtered active non-shadow internal merchants, and safely derives each merchant URL and hostname from the returned full `domain`. Catalog-disabled merchants remain available for endpoint resolution with an empty `description`; Catalog intent matching skips those rows individually without rejecting the list
