...
- Anonymous merchant-route discovery with `clink tool internal-ucp get-merchant-list`: every environment calls its `GET /agent/ucp/merchants`, uses the server-filtered active non-shadow internal routes, and safely preserves each returned `domain` URL and its optional path while deriving the hostname. One merchant ID may have multiple routes. Catalog-disabled or isolated malformed/unmatchable rows are skipped individually, and a hostname assigned to different merchant IDs is removed without hiding unrelated routes; no environment reads a static or bundled merchant list
...
- VIC agentic authorization preparation (Visa readiness check, Instruction reuse/create, CLI-owned no-card PENDING continuation, and page-driven completion)
...
- Event-driven async completion — keeps the initiating CLI process waiting for exact Instruction/card activation, refund, VIC, or post-3DS events instead of ending after a link or busy-retrying
