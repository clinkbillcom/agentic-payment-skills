...
- Anonymous supported-merchant discovery with `clink ucp-merchant list --internal`: it calls the selected environment's `GET /agent/ucp/merchants`, uses the server-filtered active merchants, and safely derives each merchant URL and hostname from the returned full `domain`
