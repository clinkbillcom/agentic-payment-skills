...
Publisher uses 1–128 Unicode letters, combining marks, digits, `.`, `_`, or `-`, so names such as Chinese publishers are preserved. Skill name uses the same characters and may also contain internal ASCII spaces; quote the complete package operand when spaces or non-ASCII characters are present. Version additionally allows ASCII `+`. Preserve spelling, Unicode text, spaces, and case. Reject leading/trailing spaces, path separators inside either segment, dangling `@`, extra path segments, URLs, partial-token matches, and noncanonical version syntax instead of silently falling back to latest.
