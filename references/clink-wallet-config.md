...
After OAuth succeeds, `wallet init` may refresh cached payment methods and return a `bindingUrl`, but wallet initialization never owns card setup. Never emit that init copy and never start a watched `card binding-link` from the init result. Return wallet ready with `cardReadiness=missing` when a successful refresh reports zero methods; report `paymentMethodsCacheError` without undoing OAuth success when refresh fails. A later authorized payment owns the no-card decision through the CLI's foreground PENDING Instruction continuation.
...
- `RETURN_WALLET_READY`: accept the final OAuth evidence whether the refreshed card count is positive, zero, or unknown; do not require an `oauthRequired` field from `wallet init`. A zero count returns `cardReadiness=missing` but starts no binding command and emits no card URL. A nullable `pendingInstructionId` does not start a Skill-side continuation; `instruction prepare` remains authoritative.
- `SURFACE_ERROR`: return the terminal CLI error without inventing recovery.

## Pending Instruction Continuation

Login choice follows the same delivery gate: retain `--email <email>` for email/OTP, or omit it for Portal/Google once verified. When a resolved, authorized purchase target has `walletGate=REQUIRE_STATUS`, first finish wallet login and run `classifyInstructionRestriction` over the complete frozen intent; only `CONTINUE_INSTRUCTION_CREATION` may proceed. Never start this for `CATALOG_SEARCH`, pre-selection `CATALOG_PURCHASE`, or an unattended run.
