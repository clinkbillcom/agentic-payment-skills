...
Assemble the context from the frozen purchase intent: `--title` from the product name, mandate `amountLimit` from the order total, `currencyCode` from the order currency, a short mandate `description` of what the purchase covers, and merchant name/category from the merchant context. Every mandate object must carry `description`, `amountLimit`, and `currencyCode`: the CLI pre-validates these locally and exits 2 naming the offending field — a missing or blank text field, a non-positive `amountLimit`, more than 2 decimal places, or a malformed mandate `effectiveUntilTime` all fail before any request is sent. The backend enforces the same rules but masks them behind a generic `OAuth request is invalid` error, so when that error still appears, fix the request contract rather than retrying login. Once the request passes that validation, login never fails because of the instruction context: if backend creation fails, the token response simply omits the id, `data.pendingInstructionId` comes back null, and the regular authorization flow takes over.
