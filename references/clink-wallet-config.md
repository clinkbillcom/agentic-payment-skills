...
Do not read or execute this workflow merely because the user is discovering a product. `CATALOG_SEARCH` returns `walletGate=SKIP`; `CATALOG_PURCHASE` returns `walletGate=DEFER_UNTIL_SELECTION` while candidates are being found. Both stages are anonymous and run without `wallet status` or `wallet init`. Enter this reference only for an explicit wallet operation or after a resolved checkout/payment contract returns `walletGate=REQUIRE_STATUS`.
...
Record `data.pendingInstructionId` from the init envelope. Null means only that no usable Quick ID was returned: CWallet uses the same absence for a deliberate strong-auth skip and for a swallowed creation failure, so return to the regular authorization resolver rather than assuming a list is required. On the first-card path, `payment_method.added` proves only card addition. Extract its exact `paymentInstrumentId`, refresh the card list, and run `classifyQuickInstructionActivationGate`. A card with a Quick ID, `strongAuthReady=false`, and `authProtocol=VISA|MASTERCARD` gets one same-card `singleAttempt` readiness poll before exact instruction verification. Merge the poll result's `strongAuthReadinessWaitAttempted=true` continuation after refresh, so event, timeout, non-ready/wrong-card event, empty result, or poll gap cannot start a second wait. Quick activation likewise gets one bounded poll and final GET with `activationWaitAttempted=true`; final PENDING returns to the regular authorization list. Missing-ID, unsupported-protocol, and still-not-ready branches return to the regular resolver; malformed capability data fails closed. On the already-ready path (cards exist, no fresh binding ceremony) the ID is informational only — never start a binding-driven Quick wait there.
...
- Setup required: neither mode is complete. Collect missing setup input before starting a `REQUIRE_STATUS` operation. `CATALOG_SEARCH` and pre-selection `CATALOG_PURCHASE` are not blocked by this state.
...
For authorization routing, treat `strongAuthReady` (Boolean) plus `authProtocol` (`VISA` or `MASTERCARD`) as the only authoritative payment-method contract. Do not infer readiness from card brand or legacy network-specific registration booleans. `strongAuthReady=false` bypasses instruction matching; missing/invalid/conflicting readiness, or `strongAuthReady=true` without one supported protocol, fails closed.
