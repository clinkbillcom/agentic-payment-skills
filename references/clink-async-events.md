...
Quick instruction setup chains two waits in a fixed order: first the binding watch's `payment_method.added` envelope, then the instruction activation wait for `purchase_instruction.activated`. Feed the recorded `pendingInstructionId` through `classifyQuickInstructionActivationGate` only after the binding envelope arrives — the gate reuses the instruction-activation waitSpec, so verification and polling follow the same `clink instruction get` plus `clink events poll --type purchase_instruction.activated --no-ack --format json` pattern. Never start the activation wait without a fresh binding ceremony: a wallet that was already ready produces no activation event.
