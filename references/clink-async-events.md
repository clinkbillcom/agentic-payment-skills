...
| UCP checkout payment success | `agent_order.succeeded` with exact matching `checkoutId`; after checkout complete returns `completed` or `complete_in_progress`, poll with `clink events poll --type agent_order.succeeded --checkout-id <checkoutId> [--ucp-order-id <frozenUcpOrderId>] [--endpoint <originalEndpoint>] --max-wait 900 --format json`. On a match, that same CLI process fetches the UCP order. |
