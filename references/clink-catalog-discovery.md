...
- Keep each candidate's target identity intact. Every present product ID/name/URL alias must be non-empty and consistent, `merchantId` and `storeId` are mutually exclusive, and a store requires `channelType`. A store ordering URL's `product_id` values must all equal the candidate product ID. Preserve the selected candidate's price/currency/quantity facts; never invent a `merchantId` for a platform store.
