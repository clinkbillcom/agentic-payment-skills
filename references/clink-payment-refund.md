...
Card brand is display data; `strongAuthReady` and `authProtocol` determine the authorization route. An unattended run with false/missing readiness returns `SURFACE_UNATTENDED_AUTHORIZATION_GAP` and never bypasses authorization.
...
For the strong-auth-ready Visa/Mastercard branch, run:
