...
This preserves email/OTP. For the Main Edition only, the target Portal/Google recipe after the delivery gate in `references/clink-wallet-config.md` is:

```bash
bin/clink wallet init --open --format json
```

The local Skill classifiers support the no-email recipe, but a new Main bundle and Portal/backend integration verification remain required. Do not silently change a Google request to OTP while delivery is pending. Use the Skill-local `loginMethod` contract in `references/clink-wallet-config.md`; never add a `--login-method` flag or provider-specific URL parameters.
...
That state is not self-announcing. `wallet status` reports `authorizationEnvironmentMatches: true` whenever the stored `issuerOrigin` agrees with the effective `baseUrl`, even when a saved production wallet is used through this sandbox/UAT wrapper. A wallet can therefore report fully OAuth-ready while pointed at the wrong environment.
...
For every wallet initialization, pass `--open`. Pass `--email <email>` to preserve the old email/OTP flow; after the delivery gate passes, omit `--email` to open Portal with a Google button. Do not inject a cached email into the no-email path or ask for an email unless the user chose email/OTP and one is missing. There is no `--name` flag: `wallet init --name` exits 2. The email path derives the initial name from the email text before `@`; the Google path consumes Clink's verified identity result. `config set name` changes the local display name later. When live stderr prints `Opening your browser...`, the CLI has requested that the system browser handle the complete URL; that line proves neither that a visible window opened nor that OAuth polling has begun. Keep reading until the current attempt prints the complete `Waiting for authorization...` marker, then mark its Clink device-token poll active, tell the user to complete the selected Portal login, and leave that same process running without repeating the URL. Email/OTP retains verification and confirmation; Google login adds no extra OTP. If browser launch fails, wait for that same marker and read the URL only from the current process's latest wallet-init attempt segment. Never poll Google or start Event Hub polling for OAuth.

An affirmative fresh-login request is different from asking for the same URL again. `重新登录`, `再登录一次`, `重新授权钱包`, an expired/missed login link, `log in again`, and `fresh login link` start exactly one new `wallet init` process. The CLI generation makes the new process authoritative and causes the older attempt to stop. Resolve email from the current request before wallet status only for the chosen email/OTP path; a cached email must not override a Google choice. Never let an already-ready status suppress explicit re-login. Do not use chat history, terminal scrollback, or an older child process as the URL source.

Customer identity comes from the server-verified email and reuses the existing `customerId` for that email. No Google `sub` binding table, `login_ui`, `expectedEmail`, or extra Google OTP is part of the contract. Preserve the CLI-returned URL verbatim; do not synthesize identity fragments or provider parameters. Portal/server-side Google authorization and the original Clink device-token poll remain separate responsibilities.

**`--no-open` belongs on the legacy link-producing commands, not `wallet init`.** `card binding-link`, `card setup-link`, `card modify-link`, `risk link`, `instruction create`, `instruction sign-url`, `instruction update`, and `instruction cancel` all launch a browser when `--open` or a stored `default-open-links` says so. `--no-open` suppresses launch only; the built-in event watch is controlled separately by `--no-watch` and must stay on. `instruction prepare` is the exception: it never auto-opens and rejects both `--open` and `--no-watch`, so pass neither open flag.
...
Before OAuth completion, the running `wallet init` process polls the OAuth device-token endpoint, not the Event Hub. Treat that poll as active only after the current attempt prints the complete `Waiting for authorization...` marker; do not start `events poll` for OAuth. Init's final `data.bindingUrl` came from a cache refresh with watching disabled and must never be sent or used to start a separate binding flow. Return OAuth readiness even when `paymentMethodCount=0`; the authorized payment's foreground `instruction prepare` command owns any later Bind Card prompt and exact-ID activation wait.

For that PENDING continuation, run `clink instruction prepare ... --max-wait 900 --format json` and keep the same CLI process in the foreground. Do not pass `--open` or `--no-watch`; both are rejected. The first envelope is safe to surface only when non-empty `instructionId`, `status=PENDING`, `watchReady=true`, `watchEventType=purchase_instruction.activated`, `processRunning=true`, and `terminal=false` accompany the trusted `bindingUrl`. Do not treat it as command completion. The final envelope must preserve the same ID and report `command=instruction prepare`, `status=ready`, `instructionStatus=ACTIVE`, and non-empty `instruction.paymentInstrumentId`. A timeout may expose only its environment-locked same-ID read-only `resumeCommand`; external process death before the final envelope permits only an exact-ID read-only `instruction get`. Never run prepare again, Checkout, or payment.
