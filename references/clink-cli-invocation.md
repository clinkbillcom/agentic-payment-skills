...
The merchant-list source is different from the search API origin. This UAT distribution reads its bundled sandbox merchant-list document locally, so the list step needs no network preflight; preflight only `https://uat-api.clinkbill.com` before search.
