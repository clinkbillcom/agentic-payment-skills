...
`tool internal-ucp get-merchant-list` sends an anonymous `GET /agent/ucp/merchants` to that same environment's Catalog API origin. Preflight the selected API origin before the merchant-list command; the successful probe is reusable for the later search calls to that exact origin in the same workflow. Production, sandbox/UAT, and test all use this API; the command never reads a static public document or a bundled merchant list.
