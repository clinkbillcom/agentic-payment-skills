...
const SKILL_LIST_CONTEXT_TTL_MS = 2 * 60 * 60 * 1000;
...
function normalizeSkillListRow(row = {}) {
  ...
  const versionNo = normalizedString(row.versionNo ?? row.version_no ?? row.version);
...
function contextIdentity(value = {}) {
  return {
    userId: normalizedString(value.userId ?? value.user_id),
    conversationId: normalizedString(
...
function recentDisplayedSnapshot(context = {}) {
  ...
  const candidates = snapshots.map((snapshot) => {
    ...
    if (!sameContextIdentity(context, snapshot)) return null;
    const displayedAt = timestampMs(
      snapshot.displayedAtMs ?? snapshot.displayed_at_ms ?? snapshot.displayedAt ?? snapshot.displayed_at,
    );
    if (displayedAt === null) return null;
    const age = now - displayedAt;
    if (age < 0 || age > SKILL_LIST_CONTEXT_TTL_MS) return null;
    const rows = normalizeRows(snapshot.rows);
    if (rows === null) return null;
    return { ...snapshot, displayedAtMs: displayedAt, rows };
  }).filter(Boolean);

  return candidates.sort((left, right) => right.displayedAtMs - left.displayedAtMs)[0] ?? null;
}
...
export function classifySkillTipPrerequisites(input = {}) {
  ...
  const snapshot = recentDisplayedSnapshot(context);
