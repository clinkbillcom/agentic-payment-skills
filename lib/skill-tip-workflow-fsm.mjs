...
const ACCOUNT_EVENT_POLL_COMMAND = 'clink-cli events poll --type account-created,account-reloaded --max-wait 60 --format json';
const CARD_BINDING_REQUIRED_MESSAGE = 'Credit 余额不足，请先绑定银行卡';
...
export const SkillTipState = Object.freeze({
  ...
  TIP_CARD_BINDING_REQUIRED: 'TIP_CARD_BINDING_REQUIRED',
...
export const SkillTipAction = Object.freeze({
  ...
  START_CARD_BINDING: 'START_CARD_BINDING',
...
function observationError(observation = {}) {
  for (const source of [observation.stderr, observation.stdout, observation.result]) {
    const parsed = parseMaybeJson(source, 'first');
    if (typeof parsed === 'string') {
      const message = normalizedString(parsed);
...
function isCardBindingRequiredError(error) {
  return Number(error?.code) === 402
    && error?.message === CARD_BINDING_REQUIRED_MESSAGE;
}

export function classifySkillListObservation(observation = {}, presentation = {}) {
  ...
  return {
    ...
    rows,
    table: skillListTable(rows, presentation),
    snapshot: { scope: 'tippable', rows },
  };
}
...
function identityCommand(target, amount) {
  return `clink-cli skills tip --publisher ${shellWord(target.publisher)}`
    + ` --name ${shellWord(target.skillName)}`
    + ` --amount ${shellWord(amount)} --format json`;
}

function versionlessTipTarget(target = {}) {
  return {
...
export function classifySkillTipPrerequisites(input = {}) {
  ...
  const resolvedTarget = versionlessTipTarget(resolvedIdentity(row));
  ...
  if (confirmationRequired) {
    ...
    return {
      ...
      confirmationPrompt: `确认打赏第 ${number} 号 ${resolvedTarget.publisher}/${resolvedTarget.skillName} ${amount} USD 吗？`,
    };
  }
...
export function classifySkillTipObservation(observation = {}) {
  ...
  if (isCardBindingRequiredError(error)) {
    return {
      state: SkillTipState.TIP_CARD_BINDING_REQUIRED,
      action: SkillTipAction.SURFACE_ERROR,
      terminal: true,
      reason: 'skill_tip_default_balance_insufficient',
      paymentStatus: 'NOT_PAID',
      userMessage: CARD_BINDING_REQUIRED_MESSAGE,
      forbiddenRecovery: 'RECHARGE_BALANCE',
      error,
      exitCode,
    };
  }
  ...
  if (data.status === 'paid') {
    ...
    const expectedResource = paymentExpectedResource(data, observation);
    const accountWaitSpecs = ['account-created', 'account-reloaded'].map((eventType) => ({
      eventType,
      maxWaitSeconds: 60,
      noAck: false,
