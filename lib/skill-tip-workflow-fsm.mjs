...
export function classifySkillTipPrerequisites(input = {}) {
  ...
  if (refreshedRows === null) {
    return {
      ...
      action: SkillTipAction.SURFACE_ERROR,
      terminal: true,
      reason: 'invalid_skill_number_refresh',
    };
  }
  ...
  return {
    ...
    command: `clink-cli skills tip --number ${number} --expected-skill-id ${shellWord(refreshed.skillId)} --amount ${shellWord(amount)} --format json`,
...
function tipAuthorizationBindingMatches(data = {}, expectedTip = {}) {
  ...
  const expectedAmount = positiveAmount(expectedTip.amount);
  ...
  const actualAmount = positiveAmount(data.amount);
  const actualCurrency = normalizedString(data.currency)?.toUpperCase();
  return (expectedSkillId === null || actualSkillId === expectedSkillId)
    && (expectedAmount === null || Number(actualAmount) === Number(expectedAmount))
...
export function classifySkillTipObservation(observation = {}) {
  ...
  if (data.status === 'paid') {
    ...
    if (expectedTip && !tipAuthorizationBindingMatches(data, expectedTip)) {
      return {
        state: SkillTipState.TIP_PAYMENT_UNKNOWN,
        action: SkillTipAction.VERIFY_BEFORE_RETRY,
        terminal: false,
