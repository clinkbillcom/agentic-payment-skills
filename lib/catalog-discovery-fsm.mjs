...
function broadSearchCommand(query, channelType, country) {
  ...
  return `clink-cli catalog search --query ${shellQuoteIfNeeded(query)}${channelArgument}${contextArgument} --format json`;
}
...
function broadSearchStep(input, query, previousReason) {
  ...
  if (raw === undefined) {
    return {
      ...
      storeId,
      country,
      command: broadSearchCommand(query, channelType, country),
    };
  }
