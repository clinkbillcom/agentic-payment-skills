...
function absoluteHttpOrigin(value) {
  const url = absoluteHttpUrl(value);
  if (!url) return null;
...
function merchantCandidatesOf(data) {
  // `ucp-merchant list --internal` returns the anonymous API array in the CLI success
  // envelope's `data`. Keep the former `{ merchants: [...] }` shape as an input adapter for
  // callers with an in-flight discovery snapshot from an older Skill version.
  const apiArray = Array.isArray(data);
  const raw = apiArray ? data : data?.merchants;
  if (!Array.isArray(raw)) return null;
  if (apiArray && raw.some((entry) => (
    !entry
    || typeof entry !== 'object'
    || Array.isArray(entry)
    || typeof entry.merchant_id !== 'string'
    || !entry.merchant_id.trim()
    || typeof entry.merchant_name !== 'string'
    || !entry.merchant_name.trim()
    || typeof entry.description !== 'string'
    || !absoluteHttpOrigin(entry.domain)
  ))) return null;
  return raw
    .filter((entry) => entry && typeof entry === 'object' && !Array.isArray(entry))
    .map((entry) => {
      const apiDomainPresent = entry.domain !== undefined;
      const apiMerchantUrl = apiDomainPresent ? absoluteHttpOrigin(entry.domain) : null;
      const legacyDomainName = normalizedString(entry.domain_name ?? entry.domainName)
        ?.toLowerCase()
        .replace(/\.+$/u, '') ?? null;
      const legacyMerchantUrl = apiDomainPresent
        ? null
        : absoluteHttpUrl(entry.merchant_url ?? entry.merchantUrl);
      const apiDomainName = domainFromUrl(apiMerchantUrl);
      const apiDomainConsistent = !legacyDomainName || legacyDomainName === apiDomainName;
      const merchantUrl = apiDomainPresent
        ? (apiDomainConsistent ? apiMerchantUrl : null)
        : legacyMerchantUrl;
      const domainName = apiDomainPresent
        ? (merchantUrl ? apiDomainName : null)
        : legacyDomainName;
      return {
        ...
        domainName,
        merchantUrl: merchantUrl && (!domainName || domainFromUrl(merchantUrl) === domainName)
          ? merchantUrl
          : null,
        description: normalizedString(entry.description),
        enabled: apiArray || entry.enabled !== false,
      };
    })
...
function merchantDiscriminatorOf(input = {}) {
  const merchantUrl = absoluteHttpUrl(input.merchant_url ?? input.merchantUrl);
  const domainName = normalizedString(
    input.domain_name
      ?? input.domainName
      ?? input.merchant_domain
      ?? input.merchantDomain,
  )?.toLowerCase().replace(/\.+$/u, '') ?? domainFromUrl(merchantUrl);
  return { merchantUrl, domainName };
}
...
function merchantListCommand(environmentFlag) {
  ...
  return `clink ucp-merchant list --internal${flagArgument} --format json`;
}
