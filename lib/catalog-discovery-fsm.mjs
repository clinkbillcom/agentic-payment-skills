...
function merchantScopedSearchCommand(merchantId, query, language) {
  const languageArgument = language
    ? ` --language ${shellQuoteIfNeeded(language)}`
    : '';
  return `clink ucp-catalog search --merchant-id ${shellQuoteIfNeeded(merchantId)}`
    + ` --query ${shellQuoteIfNeeded(query)}${languageArgument} --format json`;
}
...
function broadSearchCommand(query, channelType, country, language) {
  ...
  return `clink catalog search --query ${shellQuoteIfNeeded(query)}${languageArgument}${channelArgument}${contextArgument} --format json`;
}
