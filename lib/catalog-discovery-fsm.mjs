  ...
  return raw
    ...
    .map((entry) => {
      const domainName = normalizedString(entry.domain_name ?? entry.domainName)
        ?.toLowerCase()
        .replace(/\.+$/u, '') ?? null;
      const merchantUrl = absoluteHttpUrl(entry.merchant_url ?? entry.merchantUrl);
      return {
        merchantId: normalizedString(entry.merchant_id ?? entry.merchantId),
        domainName,
        merchantUrl: merchantUrl && domainFromUrl(merchantUrl) === domainName
          ? merchantUrl
          : null,
...
function uniqueMerchantCandidate(merchants, merchantId, discriminator = {}) {
  ...
  const discriminated = matches.filter((entry) => (
    (!merchantUrl || entry.merchantUrl === merchantUrl)
