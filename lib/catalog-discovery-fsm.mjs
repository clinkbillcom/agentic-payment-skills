...
export function resolveCatalogLanguage(input = {}) {
  ...
  for (const rawLanguage of rawLanguages) {
    try {
      const [catalogLanguage] = Intl.getCanonicalLocales(rawLanguage);
      if (!catalogLanguage) throw new RangeError('empty language');
...
function catalogContextArgument(country, catalogLanguage) {
  const context = {};
  if (country) context.address_country = country;
  if (catalogLanguage) context.language = catalogLanguage;
...
function merchantScopedSearchCommand(
  ...
  const flagArgument = environmentFlag ? ` ${environmentFlag}` : '';
  const contextArgument = catalogContextArgument(null, catalogLanguage);
  return `clink ucp-catalog search --merchant-id ${shellQuoteIfNeeded(merchantId)}`
    + ` --query ${shellQuoteIfNeeded(query)}${contextArgument}${flagArgument} --format json`;
}
...
function broadSearchCommand(
  ...
  const contextArgument = catalogContextArgument(country, catalogLanguage);
  const flagArgument = environmentFlag ? ` ${environmentFlag}` : '';
  return `clink catalog search --query ${shellQuoteIfNeeded(query)}${channelArgument}${contextArgument}${flagArgument} --format json`;
}
