...
function inspectVerificationUrl(value) {
  ...
  const promptIndex = value.indexOf(VERIFICATION_URL_PROMPT);
  if (promptIndex < 0) {
    return {
      present: false,
      lineComplete: false,
...
export function classifyWalletInitObservation(observation = {}) {
  ...
  if (verificationUrl.authorizationUrl && isRunning) {
    ...
    if (browserOpened) {
      return {
        state: WalletWorkflowState.OAUTH_AUTHORIZATION_REQUIRED,
        action: WalletWorkflowAction.TELL_USER_BROWSER_OPENED_AND_WAIT,
        terminal: false,
        reason: 'wallet_init_oauth_browser_opened',
        browserOpened: true,
