...
export const WalletWorkflowAction = Object.freeze({
  ...
  START_WATCHED_CARD_BINDING: 'START_WATCHED_CARD_BINDING',
  START_WALLET_SETUP: 'START_WALLET_SETUP',
  SURFACE_ERROR: 'SURFACE_ERROR',
});

const WATCHED_CARD_BINDING_COMMAND =
...
function inspectVerificationUrl(value, loginMethod) {
  ...
  try {
    ...
    const emailRequired = loginMethod === 'email' || loginMethod === 'otp';
    const portalLogin = loginMethod === 'portal' || loginMethod === 'google';
    // Main Portal login preserves the server URL, including opaque query/fragment context.
    const legacyShape = hasNonEmptyString(parsed.searchParams.get('user_code'))
      && ((!emailRequired && parsed.hash === '') || (
...
export function classifyWalletInitObservation(observation = {}) {
  ...
  if (
    ...
    const pendingInstructionId = hasNonEmptyString(data.pendingInstructionId)
      ...
      : null;
    if (
      data.paymentMethodsCached === true
      && paymentMethodCount === 0
      && hasNonEmptyString(data.bindingUrl)
    ) {
      return {
        state: WalletWorkflowState.WALLET_INITIALIZED,
        action: WalletWorkflowAction.START_WATCHED_CARD_BINDING,
        terminal: false,
        reason: 'wallet_init_succeeded_card_binding_watch_required',
        walletReady: true,
        bindingUrlRequired: true,
        emitUrl: false,
        pendingInstructionId,
        command: WATCHED_CARD_BINDING_COMMAND,
        handoffRequirements: {
          watchReady: true,
    ...
    return {
      ...
      cardReadiness: cardReadinessKnown
        ? (paymentMethodCount > 0 ? 'ready' : 'unknown')
        : 'unknown',
      data,
    };
  }
  ...
  const verificationUrl = inspectVerificationUrl(stderr, observation.loginMethod);
