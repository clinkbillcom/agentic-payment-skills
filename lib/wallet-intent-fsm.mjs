...
function emailFrom(input = {}, text = '', includeCached = false) {
  const candidates = [
...
export function classifyWalletIntent(input = {}) {
  ...
  const requestedLoginMethod = input.loginMethod;
  if (
    requestedLoginMethod !== undefined
    && !['email', 'otp', 'portal', 'google'].includes(requestedLoginMethod)
  ) {
    return reject('wallet_login_method_invalid');
  }

  const hasRequestedEmail = emailFrom(input, text) !== null
    || [input.email, input.walletEmail, input.wallet_email]
      .some((value) => normalizedString(value) !== null);
  const loginMethod = requestedLoginMethod === undefined
    ? (hasRequestedEmail ? 'email' : 'portal')
    : (['email', 'otp'].includes(requestedLoginMethod) ? 'email' : 'portal');
  // Cached identity is a fallback only after the email/OTP path has been selected.
  const email = loginMethod === 'email' ? emailFrom(input, text, true) : null;
