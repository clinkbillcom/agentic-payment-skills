...
function safeWalletOrigin(value) {
...
function safeExternalEndpoint(value) {
  const raw = normalizedString(value);
  if (!raw) return null;
  try {
    const parsed = new URL(raw);
...
function consistentWalletOrigin(values) {
  ...
  const origins = values.map(safeWalletOrigin);
  if (origins.some((origin) => origin === null)) {
    return {
      valid: false,
      reason: 'external_checkout_wallet_environment_unverified',
    };
  }
  if (new Set(origins).size > 1) {
    return {
      valid: false,
      reason: 'external_checkout_wallet_environment_conflict',
    };
  }
...
function walletStatusOrigin(rawStatus) {
  ...
  const baseUrlValues = [data, ...(data === parsed ? [] : [parsed])]
    .flatMap((source) => ['baseUrl', 'base_url']
      .filter((key) => hasOwn(source, key))
      .map((key) => source[key]));
  const resolution = consistentWalletOrigin(baseUrlValues);
...
function externalCheckoutWalletOrigin(input = {}) {
  const walletStatuses = ['walletStatus', 'wallet_status']
  ...
  const statusResolutions = walletStatuses.map(walletStatusOrigin);
  const invalidStatus = statusResolutions.find((resolution) => !resolution.valid);
  if (invalidStatus) return invalidStatus;

  const statusOrigins = statusResolutions.map((resolution) => resolution.value);
  ...
  const explicitBaseUrls = ['walletBaseUrl', 'wallet_base_url']
    .filter((key) => hasOwn(input, key))
    .map((key) => input[key]);
  const explicitOrigin = consistentWalletOrigin(explicitBaseUrls);
  if (!explicitOrigin.valid) return explicitOrigin;

  const walletOrigin = statusOrigins[0];
  if (explicitOrigin.value && explicitOrigin.value !== walletOrigin) {
    return {
      valid: false,
      reason: 'external_checkout_wallet_environment_conflict',
    };
  }
  return { valid: true, value: walletOrigin };
}

function routeError(reason, errorCode = null) {
  return {
...
function selectedEndpointRouteResult(input, fields, resolvedEndpoint, options) {
  const endpoint = safeExternalEndpoint(resolvedEndpoint);
  if (!endpoint) {
    return routeError(options.invalidEndpointReason, options.invalidEndpointReason);
  }
  const walletOrigin = externalCheckoutWalletOrigin(input);
  if (!walletOrigin.valid) {
    return routeError(walletOrigin.reason);
  }
  if (new URL(endpoint).origin !== walletOrigin.value) {
    return routeError(
      'checkout_endpoint_wallet_environment_conflict',
      'checkout_endpoint_wallet_environment_conflict',
    );
  }
  return {
    ...
    terminal: false,
    ...fields,
    walletOrigin: walletOrigin.value,
    endpoint,
  };
}

function externalRouteResult(input, fields, resolvedEndpoint = null) {
  const walletOrigin = externalCheckoutWalletOrigin(input);
  if (!walletOrigin.valid) return routeError(walletOrigin.reason);

  return selectedEndpointRouteResult(
    input,
    fields,
    resolvedEndpoint ?? `${walletOrigin.value}/agent/ucp/external`,
    {
      ...
      invalidEndpointReason: 'invalid_standard_ucp_external_endpoint',
    },
  );
}

function internalRouteResult(input, fields, resolvedEndpoint, invalidEndpointReason) {
  return selectedEndpointRouteResult(input, fields, resolvedEndpoint, {
...
function classifyProfileCandidate(input, productUrl, merchantDomain, profileUrl) {
  ...
  if (endpointData.provider) {
    if (endpointData.provider.toLowerCase() === 'clinkbill') {
      return internalRouteResult(input, {
        reason: 'internal_ucp_clinkbill_provider',
        merchantDomain,
        profileUrl,
        provider: endpointData.provider,
      }, endpointData.endpoint, 'invalid_standard_ucp_internal_endpoint');
    }
    return externalRouteResult(input, {
...
export function classifyUcpCheckoutRoute(input = {}) {
  ...
  if (internalEndpoint.status === 'resolved') {
    ...
    }, internalEndpoint.endpoint, 'invalid_internal_ucp_endpoint_output');
  }
