...
export function classifyUcpCheckoutRoute(input = {}) {
  ...
  if (!internalEndpoint.observed) {
    const sandboxObservation = booleanValue(input.sandbox ?? input.isSandbox ?? input.is_sandbox);
    const sandbox = sandboxObservation !== false;
    return {
      ...
      productUrl,
      command: internalEndpointCommandForUrl(productUrl, sandbox),
    };
  }
