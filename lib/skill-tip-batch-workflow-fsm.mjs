...
} from './skill-tip-workflow-fsm.mjs';
...
function validatePendingItems(pending) {
  ...
  for (const item of pending.items) {
    ...
    const identityKey = `${publisher?.toLowerCase()}\u0000${skillName?.toLowerCase()}`;
    if (
      !itemId
      || itemIds.has(itemId)
      || !publisher
...
function validateBatchProgress(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const batchId = normalizedString(value.batchId ?? value.batch_id);
  const currency = normalizedString(value.currency)?.toUpperCase();
  const authorizedTotal = canonicalPositiveAmount(value.authorizedTotal ?? value.authorized_total);
  const currentIndex = Number(value.currentIndex ?? value.current_index);
  const executionItems = value.executionItems ?? value.execution_items;
  const results = value.results;
  const optionalAccountWatches = value.optionalAccountWatches ?? value.optional_account_watches ?? [];
  if (
    !batchId
    ...
    || !Array.isArray(executionItems)
    || executionItems.length === 0
    || currentIndex < 0
    || currentIndex >= executionItems.length
    || !Array.isArray(results)
    || results.length !== currentIndex
    || !Array.isArray(optionalAccountWatches)
  ) return null;
  ...
  for (const item of executionItems) {
    ...
    const skillName = normalizedString(item?.skillName ?? item?.skill_name);
    const amount = canonicalPositiveAmount(item?.amount);
    const single = publisher && skillName && amount
      ? classifySkillTipPrerequisites({
        tip: {
      ...
      : null;
    const expectedSkillId = normalizedString(item?.skillId ?? item?.skill_id);
    const actualExpectedTip = item?.expectedTip;
    const expectedBindingMatches = actualExpectedTip
      && normalizedString(actualExpectedTip.publisher) === publisher
      && normalizedString(actualExpectedTip.skillName ?? actualExpectedTip.skill_name) === skillName
      && canonicalPositiveAmount(actualExpectedTip.amount) === amount
      && normalizedString(actualExpectedTip.currency)?.toUpperCase() === 'USD'
      && normalizedString(actualExpectedTip.skillId ?? actualExpectedTip.skill_id) === expectedSkillId;
    if (
      !itemId
      || itemIds.has(itemId)
      || !publisher
      || !skillName
      || !amount
      || normalizedString(item?.currency)?.toUpperCase() !== 'USD'
      || normalizedString(item?.command) !== single?.command
      || !expectedBindingMatches
    ) return null;
    itemIds.add(itemId);
  }
  if (addCanonicalAmounts(executionItems.map((item) => canonicalPositiveAmount(item.amount))) !== authorizedTotal) {
    return null;
  }
  for (const [index, result] of results.entries()) {
    if (
      !result
      || result.itemId !== executionItems[index].itemId
      || !['PAID', 'FAILED', 'UNKNOWN'].includes(result.paymentStatus)
    ) return null;
  }
  return {
    ...value,
    ...
    executionItems,
    results,
    optionalAccountWatches,
  };
}

function terminalPaymentStatus(itemWorkflow) {
  if (itemWorkflow.paymentStatus === 'PAID' && itemWorkflow.paymentTerminal === true) return 'PAID';
  if (itemWorkflow.paymentStatus === 'FAILED') return 'FAILED';
  if (itemWorkflow.paymentStatus === 'UNKNOWN') return 'UNKNOWN';
  if (itemWorkflow.terminal === true && itemWorkflow.paymentStatus === 'NOT_PAID') return 'FAILED';
  return null;
}

function sanitizedItemResult(item, itemWorkflow, paymentStatus) {
  return {
    itemId: item.itemId,
    publisher: item.publisher,
    skillName: item.skillName,
    ...
    currency: 'USD',
    paymentStatus,
    state: itemWorkflow.state,
    action: itemWorkflow.action,
    reason: itemWorkflow.reason,
    ...(itemWorkflow.payment ? { payment: itemWorkflow.payment } : {}),
    ...(itemWorkflow.accountEventStatus
      ? { accountEventStatus: itemWorkflow.accountEventStatus }
      : {}),
    ...(itemWorkflow.exitCode !== undefined ? { exitCode: itemWorkflow.exitCode } : {}),
  };
}

function completedBatch(progress) {
  const counts = {
    total: progress.results.length,
    paid: progress.results.filter((item) => item.paymentStatus === 'PAID').length,
    failed: progress.results.filter((item) => item.paymentStatus === 'FAILED').length,
    unknown: progress.results.filter((item) => item.paymentStatus === 'UNKNOWN').length,
  };
  const aggregateOutcome = counts.paid === counts.total
    ? 'ALL_PAID'
    : (counts.paid > 0 ? 'PARTIAL' : 'NONE_PAID');
  return {
    state: SkillTipBatchState.BATCH_COMPLETED,
    action: SkillTipBatchAction.RETURN_SKILL_TIP_BATCH_RESULT,
    terminal: true,
    reason: 'skill_tip_batch_completed',
    intent: 'SKILL_TIP_BATCH',
    batchId: progress.batchId,
    status: 'COMPLETED',
    aggregateOutcome,
    currency: 'USD',
    authorizedTotal: progress.authorizedTotal,
    counts,
    items: progress.results,
    ignoredDuplicates: progress.ignoredDuplicates ?? [],
    optionalAccountWatches: progress.optionalAccountWatches,
  };
}
...
export function classifySkillTipBatchObservation(input = {}) {
  ...
  if (!progress) {
    return inputRequired('invalid_skill_tip_batch_progress', ['progress']);
  }
  if (input.context && !sameSkillContextIdentity(input.context, progress)) {
    return inputRequired('skill_tip_batch_progress_context_mismatch', ['context']);
  }
  ...
  const itemWorkflow = classifySkillTipObservation({
    ...
    ...(currentItem.expectedResource ? { expectedResource: currentItem.expectedResource } : {}),
  });
  const paymentStatus = terminalPaymentStatus(itemWorkflow);
  if (!paymentStatus) {
    return {
  ...
  const optionalAccountWatch = paymentStatus === 'PAID' && Array.isArray(itemWorkflow.pollCommands)
    ? {
      itemId: currentItem.itemId,
      pollCommands: itemWorkflow.pollCommands,
      accountWaitSpecs: itemWorkflow.accountWaitSpecs ?? [],
      expectedResource: itemWorkflow.expectedResource ?? {},
      paymentStatus: 'PAID',
    }
    : null;
  const nextProgress = {
    ...
    results: [
      ...progress.results,
      sanitizedItemResult(currentItem, itemWorkflow, paymentStatus),
    ],
    optionalAccountWatches: optionalAccountWatch
      ? [...progress.optionalAccountWatches, optionalAccountWatch]
      : progress.optionalAccountWatches,
  };

  if (nextProgress.currentIndex >= nextProgress.executionItems.length) {
    return {
      ...completedBatch(nextProgress),
      ...(optionalAccountWatch ? { optionalAccountWatch } : {}),
    };
  }
  const nextItem = nextProgress.executionItems[nextProgress.currentIndex];
  return {
    state: SkillTipBatchState.BATCH_IN_PROGRESS,
    action: SkillTipBatchAction.RUN_NEXT_SKILL_TIP,
    terminal: false,
    reason: 'skill_tip_batch_next_item_ready',
    command: nextItem.command,
    currentItem: nextItem,
    completedItem: nextProgress.results.at(-1),
