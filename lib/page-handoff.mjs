...
export const PageHandoffKind = Object.freeze({
  ...
  INSTRUCTION_PREPARE_CARD_BINDING: 'INSTRUCTION_PREPARE_CARD_BINDING',
...
const CONTRACTS = Object.freeze({
  ...
  [PageHandoffKind.INSTRUCTION_PREPARE_CARD_BINDING]: Object.freeze({
    actor: PageHandoffActor.USER_DEVICE_ONLY,
    reason: 'instruction_prepare_card_binding_requires_user',
    doNotAutomateReason:
      'The page collects a card number and completes VIC for one exact PENDING Instruction. The same instruction prepare process must remain in the foreground.',
    completionEvents: Object.freeze(['purchase_instruction.activated']),
    watch: 'instruction-prepare-foreground',
    singleLoad: true,
    verbatimUrl: true,
  }),
...
// The Agent Portal serves both card binding and card management from its origin root. Keep the
// path check explicit so legacy `/payment-method-setup` and `/payment-method-modify` links cannot
// be treated as trusted handoffs after the portal route changed.
const CARD_PORTAL_PATH = '/';
...
function isTrustedCardBindingUrl(value) {
  try {
    ...
    if (parsed.protocol !== 'https:'
      ...
      || parsed.username !== ''
      || parsed.password !== ''
      || parsed.pathname !== CARD_PORTAL_PATH
      || parsed.hash !== '') {
      return false;
    }
...
export function classifyPageHandoff(request = {}) {
  ...
  if (kind === PageHandoffKind.INSTRUCTION_PREPARE_CARD_BINDING) {
    const instructionId = request.instructionId ?? request.instruction_id;
    const watchReady = request.watchReady ?? request.watch_ready;
    const watchEventType = request.watchEventType ?? request.watch_event_type;
    const processRunning = request.processRunning ?? request.process_running;
    const terminal = request.terminal;
    if (!url) {
      return invalid('instruction_prepare_binding_url_missing', {
        instructionId: typeof instructionId === 'string' ? instructionId : null,
        watchReady: watchReady === true,
        watchEventType: typeof watchEventType === 'string' ? watchEventType : null,
        processRunning: processRunning === true,
        terminal: terminal === false ? false : null,
      });
    }
    if (!isTrustedCardBindingUrl(url)) {
      return invalid('instruction_prepare_binding_url_not_trusted_setup', null);
    }
    if (
      typeof instructionId !== 'string'
      || instructionId.trim().length === 0
      || watchReady !== true
      || watchEventType !== 'purchase_instruction.activated'
      || processRunning !== true
      || terminal !== false
    ) {
      return invalid('instruction_prepare_watch_not_ready', {
        instructionId: typeof instructionId === 'string' && instructionId.trim()
          ? instructionId.trim()
          : null,
        watchReady: watchReady === true,
        watchEventType: typeof watchEventType === 'string' ? watchEventType : null,
        processRunning: processRunning === true,
        terminal: terminal === false ? false : terminal === true ? true : null,
      });
    }
  }
  ...
  const base = {
    ...
    ...(kind === PageHandoffKind.INSTRUCTION_PREPARE_CARD_BINDING
      ? {
          instructionId: (request.instructionId ?? request.instruction_id).trim(),
          watchReady: true,
          watchEventType: 'purchase_instruction.activated',
          processRunning: true,
        }
