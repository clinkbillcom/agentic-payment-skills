...
export const AuthorizationWorkflowState = Object.freeze({
  ...
  VIC_READINESS_WAIT_REQUIRED: 'VIC_READINESS_WAIT_REQUIRED',
...
export const AuthorizationWorkflowAction = Object.freeze({
  ...
  WAIT_VIC_READINESS: 'WAIT_VIC_READINESS',
...
function quickVicReadinessWaitSpec(paymentInstrumentId) {
  const base = {
  ...
  if (!isVicEnabled(card)) {
    if (isUnattended(input)) {
      return {
        ...
        action: AuthorizationWorkflowAction.SURFACE_UNATTENDED_AUTHORIZATION_GAP,
        terminal: true,
        reason: 'unattended_vic_readiness_missing',
        ...baseResult(card),
      };
    }
    return {
      ...
      action: AuthorizationWorkflowAction.START_AUTHORIZATION_PREPARE_AND_WAIT,
      terminal: false,
      reason: 'visa_vic_not_ready_pending_instruction_required',
      existingPaymentInstrumentId: paymentInstrumentIdOf(card),
    };
  }
...
export function classifyQuickInstructionActivationGate(input = {}) {
  ...
  if (!card || !cardBrandText(card)) {
    if (refreshAttempted) {
      return {
        ...
        terminal: false,
        reason: !card
          ? 'quick_instruction_payment_instrument_not_found_after_refresh'
          : 'quick_instruction_card_scheme_missing_after_refresh',
        paymentInstrumentId,
    ...
    return {
      ...
      terminal: false,
      reason: !card
        ? 'quick_instruction_payment_instrument_refresh_required'
        : 'quick_instruction_card_scheme_refresh_required',
      paymentInstrumentId,
      refreshCommand: PAYMENT_INSTRUMENT_REFRESH_COMMAND,
    };
  }
  ...
  if (!isVisaCard(card)) {
    return regularResolver('new_payment_instrument_not_visa');
  }
  if (!isVicEnabled(card)) {
    if (isUnattended(input)) {
      return regularResolver('vic_readiness_unavailable_unattended');
    }
    const waitAttempted = booleanValue(
      input.vicReadinessWaitAttempted
        ?? input.vic_readiness_wait_attempted
        // Legacy aliases remain accepted for callers that used the first bounded-wait contract.
        ?? input.vicReadinessWaitTimedOut
        ?? input.vic_readiness_wait_timed_out
        ?? input.vicWaitTimedOut
        ?? input.vic_wait_timed_out,
    ) === true;
    if (waitAttempted) {
      return regularResolver('vic_readiness_not_observed_after_bounded_wait');
    }
    const waitSpec = quickVicReadinessWaitSpec(paymentInstrumentId);
    return {
      state: AuthorizationWorkflowState.VIC_READINESS_WAIT_REQUIRED,
      action: AuthorizationWorkflowAction.WAIT_VIC_READINESS,
      terminal: false,
      reason: 'quick_instruction_vic_readiness_pending',
      paymentInstrumentId,
      instructionId,
      waitSpec,
      pollCommand: waitSpec.pollCommand,
