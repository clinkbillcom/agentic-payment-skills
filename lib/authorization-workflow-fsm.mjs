...
import {
  StrongAuthProtocol,
  strongAuthCapabilityOf,
} from './strong-auth.mjs';

export { StrongAuthProtocol };
...
export const AuthorizationWorkflowState = Object.freeze({
  ...
  STRONG_AUTH_READINESS_WAIT_REQUIRED: 'STRONG_AUTH_READINESS_WAIT_REQUIRED',
...
export const AuthorizationWorkflowAction = Object.freeze({
  ...
  HANDOFF_BIND_CARD_URL_AND_AWAIT_CLI: 'HANDOFF_BIND_CARD_URL_AND_AWAIT_CLI',
  ...
  WAIT_STRONG_AUTH_READINESS: 'WAIT_STRONG_AUTH_READINESS',
...
function paymentInstrumentListCandidates(input = {}) {
...
function paymentInstrumentList(input = {}) {
  return paymentInstrumentListCandidates(input)
    .find((candidate) => Array.isArray(candidate)) || [];
}

function hasPaymentInstrumentList(input = {}) {
  return paymentInstrumentListCandidates(input).some((candidate) => Array.isArray(candidate));
}
...
function bindingUrlOf(input = {}) {
  return normalizedString(
    input.bindingUrl
      ?? input.binding_url
      ?? input.cardBindingUrl
      ?? input.card_binding_url
      ?? input.links?.bindingUrl
      ?? input.links?.binding_url,
  );
}
...
function instructionWaitSpec(instructionId, paymentInstrumentId = null, metadata = {}) {
...
const PAYMENT_INSTRUMENT_REFRESH_COMMAND =
  'clink card binding-link --no-watch --no-open --format json';

function quickStrongAuthReadinessWaitSpec(paymentInstrumentId, authProtocol) {
  const base = {
    ...
    authProtocol,
    maxWaitSeconds: 900,
    verifyCommand: PAYMENT_INSTRUMENT_REFRESH_COMMAND,
  };
  return {
    ...base,
    pollCommand: pollCommandForWaitSpec(base),
...
function baseResult(card = {}, capability = strongAuthCapabilityOf(card)) {
  const paymentInstrumentId = paymentInstrumentIdOf(card);
  return {
    ...(paymentInstrumentId ? { paymentInstrumentId } : {}),
    ...(capability.strongAuthReady === true || capability.strongAuthReady === false
      ? { strongAuthReady: capability.strongAuthReady }
      : {}),
    ...(capability.authProtocol ? { authProtocol: capability.authProtocol } : {}),
  };
}
...
export function classifyPaymentAuthorizationResolver(input = {}) {
  ...
  if (!card) {
    if (hasPaymentInstrumentList(input)) {
      ...
      return {
        state: AuthorizationWorkflowState.AUTHORIZATION_PREPARE_REQUIRED,
        action: AuthorizationWorkflowAction.START_AUTHORIZATION_PREPARE_AND_WAIT,
        terminal: false,
  ...
  const capability = strongAuthCapabilityOf(card);
  if (!capability.valid) {
    return {
      state: AuthorizationWorkflowState.AUTHORIZATION_ERROR,
      action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
      terminal: false,
      reason: `payment_instrument_${capability.reason}`,
      ...baseResult(card, capability),
    };
  }

  if (capability.strongAuthReady !== true) {
    if (isUnattended(input)) {
      return {
        state: AuthorizationWorkflowState.UNATTENDED_AUTHORIZATION_GAP,
        action: AuthorizationWorkflowAction.SURFACE_UNATTENDED_AUTHORIZATION_GAP,
        terminal: true,
        reason: 'unattended_strong_auth_readiness_missing',
        ...baseResult(card, capability),
      };
    }
    return {
      ...
      terminal: false,
      reason: capability.strongAuthReady === false
        ? 'payment_instrument_strong_auth_not_ready_bypass_authorization'
  ...
  return {
    ...
    terminal: false,
    reason: 'strong_auth_ready_list_authorizations',
    ...baseResult(card, capability),
  };
}
...
function prepareObservationEnvelopes(observation = {}) {
  const primary = observation.stdout ?? observation.data ?? observation.result ?? observation;
  const final = observation.finalStdout
    ?? observation.final_stdout
    ?? observation.finalResult
    ?? observation.final_result;
  return [primary, final]
    .filter((value) => value !== undefined && value !== null && value !== '')
    .flatMap(parseJsonEnvelopes)
    .map(unwrapCliEnvelope)
    .filter((value) => value && typeof value === 'object' && !Array.isArray(value));
}

function exactPrepareResumeCommand(data, instructionId) {
  if (
    data.resumeReadOnly !== true
    || data.createsAnotherInstruction !== false
    || data.paymentRetryAllowed !== false
  ) {
    return null;
  }
  const command = normalizedString(data.resumeCommand);
  if (!command) return null;

  const marker =
    `clink instruction get --purchase-instruction-id ${shellQuoteIfNeeded(instructionId)} --format `;
  const markerIndex = command.indexOf(marker);
  if (markerIndex < 0) return null;
  const prefix = command.slice(0, markerIndex);
  const format = command.slice(markerIndex + marker.length);
  if (format !== 'json' && format !== 'pretty') return null;
  if (!prefix) return command;
  if (!prefix.startsWith('CLINK_BASE_URL=') || !prefix.endsWith(' ')) return null;
  try {
    const baseUrl = new URL(prefix.slice('CLINK_BASE_URL='.length, -1));
    if (
...
export function classifyAuthorizationPrepareObservation(observation = {}) {
  const envelopes = prepareObservationEnvelopes(observation);
  const first = envelopes[0] ?? {};
  const firstStatus = normalizedString(first.status)?.toUpperCase() ?? '';

  if (firstStatus === 'CARD_READY' || normalizedString(first.status)?.toLowerCase() === 'card_ready') {
    return {
      state: AuthorizationWorkflowState.PAYMENT_INSTRUMENT_REFRESH_REQUIRED,
      action: AuthorizationWorkflowAction.REFRESH_PAYMENT_INSTRUMENT_LIST,
      terminal: false,
  ...
  const instructionId = instructionIdOf(first);
  const bindingUrl = bindingUrlOf(first);
  const watchReady = booleanValue(first.watchReady ?? first.watch_ready) === true;
  const watchEventType = normalizedString(first.watchEventType ?? first.watch_event_type);
  const processRunning = booleanValue(first.processRunning ?? first.process_running);
  const firstTerminal = booleanValue(first.terminal);
  if (
    firstStatus !== 'PENDING'
    || !instructionId
    || !bindingUrl
    ...
    return {
      ...
      action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
      terminal: false,
      reason: 'authorization_prepare_pending_envelope_invalid',
    };
  }

  const final = envelopes.length > 1 ? envelopes.at(-1) : null;
  if (!final && authorizationWatchEnded(observation)) {
    const verifyCommand =
      `clink instruction get --purchase-instruction-id ${shellQuoteIfNeeded(instructionId)} --format json`;
    return {
      state: AuthorizationWorkflowState.AUTHORIZATION_ACTIVATION_VERIFY_REQUIRED,
      action: AuthorizationWorkflowAction.VERIFY_AUTHORIZATION_AFTER_WATCH_GAP,
      terminal: false,
      reason: 'authorization_prepare_process_ended_before_final',
      instructionId,
      verifyCommand,
      resumeCommand: verifyCommand,
      resumeReadOnly: true,
  ...
  if (!final) {
    return {
      state: AuthorizationWorkflowState.AUTHORIZATION_ACTIVATION_WAIT_REQUIRED,
      action: AuthorizationWorkflowAction.HANDOFF_BIND_CARD_URL_AND_AWAIT_CLI,
      terminal: false,
      reason: 'pending_instruction_card_binding_required',
      instructionId,
      bindingUrl,
      watchReady: true,
      watchEventType: 'purchase_instruction.activated',
      processMustRemainRunning: true,
      processRunning: true,
  ...
  const finalInstruction =
    final.instruction && typeof final.instruction === 'object' && !Array.isArray(final.instruction)
      ? final.instruction
      : {};
  const finalIds = [
    instructionIdOf(final),
    instructionIdOf(finalInstruction),
  ].filter(Boolean);
  if (
    finalIds.length === 0
    || finalIds.some((value) => value !== instructionId)
  ) {
    return {
      state: AuthorizationWorkflowState.AUTHORIZATION_ERROR,
      action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
      terminal: false,
      reason: 'authorization_prepare_instruction_mismatch',
      instructionId,
    };
  }

  const command = normalizedString(final.command)?.toLowerCase() ?? '';
  const resultStatus = normalizedString(final.status)?.toLowerCase() ?? '';
  // The CLI can reconcile a PENDING whose card became VIC-ready without activation. Its final
  // envelope still carries the original draft, so refresh cards before resolving authorization.
  if (command === 'instruction prepare' && resultStatus === 'card_ready') {
    return {
      state: AuthorizationWorkflowState.PAYMENT_INSTRUMENT_REFRESH_REQUIRED,
      action: AuthorizationWorkflowAction.REFRESH_PAYMENT_INSTRUMENT_LIST,
      terminal: false,
  ...
  const instructionStatus = normalizedString(
    final.instructionStatus
      ?? final.instruction_status
      ?? finalInstruction.status
      ?? finalInstruction.state,
  )?.toUpperCase() ?? '';
  const paymentInstrumentId = paymentInstrumentIdOf(finalInstruction);
  ...
  if (
    ...
    return {
      ...
      action: AuthorizationWorkflowAction.RESUME_AUTHORIZED_PAYMENT,
      terminal: false,
      reason: 'authorization_prepare_active',
      instructionId,
      paymentInstrumentId,
      status: instructionStatus,
      authorization: finalInstruction,
      pendingInstructionContinuationCompleted: true,
    };
  }
  if (
    command === 'instruction prepare'
    ...
    return {
      ...
      action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
      terminal: false,
      reason: 'authorization_payment_instrument_missing',
      instructionId,
      status: instructionStatus,
    };
  }

  const pending = ['PENDING', 'CREATED', 'INPROGRESS'].includes(instructionStatus)
    || resultStatus === 'pending'
    || resultStatus === 'timeout';
  if (command === 'instruction prepare' && pending) {
    const resumeCommand = exactPrepareResumeCommand(final, instructionId);
    if (!resumeCommand) {
      return {
        state: AuthorizationWorkflowState.AUTHORIZATION_ERROR,
        action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
        terminal: false,
        reason: 'authorization_prepare_resume_invalid',
        instructionId,
      };
    }
    return {
      ...
      action: AuthorizationWorkflowAction.VERIFY_AUTHORIZATION_AFTER_WATCH_GAP,
      terminal: false,
      reason: resultStatus === 'timeout'
        ? 'authorization_prepare_timed_out'
        : 'authorization_prepare_still_pending',
      instructionId,
  ...
  return {
    ...
    action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
    terminal: false,
    reason: 'authorization_prepare_final_envelope_invalid',
    instructionId,
    status: instructionStatus,
  };
}
...
export function classifyAuthorizationActiveVerification(observation = {}, waitSpec = {}) {
  ...
  const paymentInstrumentId = paymentInstrumentIdOf(data);
  const expectedPaymentInstrumentId = paymentInstrumentIdOf(waitSpec.expectedResource || {});
  const status = statusOf(data);
  const quickActivation = (waitSpec.purpose ?? waitSpec.eventPurpose) === 'QUICK_INSTRUCTION_ACTIVATION';
  const activationWaitAttempted = booleanValue(
    waitSpec.activationWaitAttempted
      ?? waitSpec.activation_wait_attempted
      ?? waitSpec.continuation?.activationWaitAttempted
      ?? waitSpec.continuation?.activation_wait_attempted,
  ) === true;
  ...
  if (
    paymentInstrumentId
      && expectedPaymentInstrumentId
      && paymentInstrumentId !== expectedPaymentInstrumentId
  ) {
    return {
      state: AuthorizationWorkflowState.AUTHORIZATION_ERROR,
      action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
      terminal: false,
      reason: 'authorization_payment_instrument_mismatch',
      paymentInstrumentId,
      expectedPaymentInstrumentId,
      instructionId,
      status,
  ...
  if (status === 'ACTIVE') {
    if (quickActivation && expectedInstructionId && !observedInstructionId) {
      return {
        state: AuthorizationWorkflowState.AUTHORIZATION_ERROR,
        action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
        terminal: false,
        reason: 'authorization_instruction_missing',
        paymentInstrumentId,
        expectedInstructionId,
        instructionId,
        status,
    ...
    if (expectedPaymentInstrumentId && !paymentInstrumentId) {
      return {
        state: AuthorizationWorkflowState.AUTHORIZATION_ERROR,
        action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
        terminal: false,
  ...
  if (quickActivation && activationWaitAttempted) {
    return {
      state: AuthorizationWorkflowState.AUTHORIZATION_LIST_REQUIRED,
      action: AuthorizationWorkflowAction.LIST_AUTHORIZATIONS,
      terminal: false,
      reason: 'quick_instruction_still_pending_after_bounded_wait',
      paymentInstrumentId: expectedPaymentInstrumentId ?? paymentInstrumentId,
      instructionId,
      status,
    };
  }

  const nextWaitMetadata = quickActivation
    ? {
  ...
  const fallbackWaitSpec = instructionId
    ? instructionWaitSpec(instructionId, expectedPaymentInstrumentId, nextWaitMetadata)
    : {
        eventType: 'purchase_instruction.activated',
        expectedResource: waitSpec.expectedResource || {},
        ...nextWaitMetadata,
      };
  const pollCommand = quickActivation
    ? pollCommandForWaitSpec(fallbackWaitSpec)
    : waitSpec.pollCommand ?? waitSpec.poll_command ?? pollCommandForWaitSpec(fallbackWaitSpec);
  ...
  return {
    ...
    instructionId,
    status,
    waitSpec: fallbackWaitSpec,
    pollCommand,
    verifyCommand: waitSpec.verifyCommand ?? waitSpec.verify_command ?? fallbackWaitSpec.verifyCommand,
  };
}

export function classifyQuickInstructionActivationGate(input = {}) {
  const instructionId = normalizedString(
    input.pendingInstructionId
      ?? input.pending_instruction_id
      ?? input.instructionId
      ?? input.instruction_id,
  );
  const paymentInstrumentId = selectedPaymentInstrumentId(input);
  if (!paymentInstrumentId) {
    return {
      state: AuthorizationWorkflowState.AUTHORIZATION_ERROR,
      action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
      terminal: false,
      reason: 'quick_instruction_payment_instrument_id_missing',
    };
  }

  const resolverInput = { ...input, paymentInstrumentId };
  const card = choosePaymentInstrument(resolverInput);
  const refreshAttempted = booleanValue(
    input.paymentInstrumentRefreshAttempted
      ?? input.payment_instrument_refresh_attempted
      ?? input.cardRefreshAttempted
      ?? input.card_refresh_attempted,
  ) === true;
  if (!card) {
    if (refreshAttempted) {
      return {
        ...
        action: AuthorizationWorkflowAction.SURFACE_AUTHORIZATION_ERROR,
        terminal: false,
        reason: 'quick_instruction_payment_instrument_not_found_after_refresh',
        paymentInstrumentId,
        refreshCommand: PAYMENT_INSTRUMENT_REFRESH_COMMAND,
      };
    }
    return {
      state: AuthorizationWorkflowState.PAYMENT_INSTRUMENT_REFRESH_REQUIRED,
      action: AuthorizationWorkflowAction.REFRESH_PAYMENT_INSTRUMENT_LIST,
      terminal: false,
      reason: 'quick_instruction_payment_instrument_refresh_required',
      paymentInstrumentId,
      refreshCommand: PAYMENT_INSTRUMENT_REFRESH_COMMAND,
    };
  }

  const regularResolver = (quickInstructionFallbackReason) => ({
    ...classifyPaymentAuthorizationResolver(resolverInput),
    quickInstructionFallbackReason,
  });
  const capability = strongAuthCapabilityOf(card);
  if (!instructionId) {
    return regularResolver('pending_instruction_id_unavailable');
  }
  if (!capability.valid) {
    return regularResolver(`new_payment_instrument_${capability.reason}`);
  }
  if (!capability.authProtocol) {
    return regularResolver('new_payment_instrument_strong_auth_not_supported');
  }
  if (capability.strongAuthReady === false) {
    if (isUnattended(input)) {
      return regularResolver('strong_auth_readiness_unavailable_unattended');
    }
    const waitAttempted = booleanValue(
      input.strongAuthReadinessWaitAttempted
        ?? input.strong_auth_readiness_wait_attempted,
    ) === true;
    if (waitAttempted) {
      return regularResolver('strong_auth_readiness_not_observed_after_bounded_wait');
    }
    const waitSpec = quickStrongAuthReadinessWaitSpec(
      paymentInstrumentId,
      capability.authProtocol,
    );
    return {
      state: AuthorizationWorkflowState.STRONG_AUTH_READINESS_WAIT_REQUIRED,
      action: AuthorizationWorkflowAction.WAIT_STRONG_AUTH_READINESS,
      terminal: false,
      reason: 'quick_instruction_strong_auth_readiness_pending',
      paymentInstrumentId,
      authProtocol: capability.authProtocol,
      instructionId,
      waitSpec,
      pollCommand: waitSpec.pollCommand,
      refreshCommand: PAYMENT_INSTRUMENT_REFRESH_COMMAND,
  ...
  const waitSpec = instructionWaitSpec(instructionId, paymentInstrumentId, {
    purpose: 'QUICK_INSTRUCTION_ACTIVATION',
    singleAttempt: true,
    maxWaitSeconds: 900,
    activationWaitAttempted: false,
  });
  return {
    state: AuthorizationWorkflowState.AUTHORIZATION_ACTIVATION_VERIFY_REQUIRED,
    action: AuthorizationWorkflowAction.VERIFY_AUTHORIZATION_ACTIVATION,
    terminal: false,
    reason: 'quick_instruction_pending_verification',
    paymentInstrumentId,
    authProtocol: capability.authProtocol,
    instructionId,
    waitSpec,
