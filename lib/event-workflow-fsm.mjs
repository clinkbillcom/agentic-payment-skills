import { strongAuthCapabilityOf } from './strong-auth.mjs';
...
export const EventWorkflowState = Object.freeze({
  ...
  STRONG_AUTH_READY: 'STRONG_AUTH_READY',
...
export const EventWorkflowAction = Object.freeze({
  ...
  MARK_STRONG_AUTH_READY_AND_RETURN: 'MARK_STRONG_AUTH_READY_AND_RETURN',
...
function isStrongAuthReadyEvent(event = {}) {
  const payload = eventPayloadOf(event);
  const capability = strongAuthCapabilityOf(payload);
  return capability.valid && capability.strongAuthReady === true;
}
...
function waitSpecDomain(waitSpec = {}) {
  ...
  if ((waitSpec.purpose ?? waitSpec.eventPurpose) === 'STRONG_AUTH_READINESS') {
    return EventWorkflowDomain.STRONG_AUTH;
  }
  ...
  for (const key of group) {
    switch (key) {
      ...
      case 'sessionId':
        ...
        break;
      case 'paymentExecutionDetailId':
        values.push(
          payload?.paymentExecutionDetailId,
          payload?.payment_execution_detail_id,
          originalEvent?.paymentExecutionDetailId,
        );
        break;
      case 'refundOrderId':
...
function correlationGroupsFor(event = {}, workflow = {}, expectedResource = {}) {
  ...
  if (workflow.domain === EventWorkflowDomain.PAYMENT) {
    const groups = [['orderId'], ['paymentExecutionDetailId'], ['sessionId']];
    const comparable = groups.filter((group) => (
      expectedGroupValues(expectedResource, group).length > 0
      && eventGroupValues(event, group, group[0] === 'orderId').length > 0
    ));
    if (comparable.length > 0) return comparable;
    const firstExpected = groups.find(
      (group) => expectedGroupValues(expectedResource, group).length > 0,
    );
    return firstExpected ? [firstExpected] : [];
  }
  if (workflow.domain === EventWorkflowDomain.REFUND) return [['refundOrderId', 'refundId']];
  if (workflow.domain === EventWorkflowDomain.STRONG_AUTH) {
...
export function classifyEventWorkflow(event = {}) {
  ...
  switch (type) {
    ...
    case 'payment_method.update':
      if (isStrongAuthReadyEvent(event)) {
        return {
          domain: EventWorkflowDomain.STRONG_AUTH,
          state: EventWorkflowState.STRONG_AUTH_READY,
          action: EventWorkflowAction.MARK_STRONG_AUTH_READY_AND_RETURN,
          terminal: true,
          reason: `${type}_strong_auth_ready`,
        };
      }
      return {
        domain: EventWorkflowDomain.PAYMENT_METHOD,
...
export function classifyEventPollObservation(observation = {}, waitSpec = {}) {
  ...
  const actionableEvents = (normalized.purpose ?? normalized.eventPurpose) === 'STRONG_AUTH_READINESS'
    ? targetEvents.filter((event) => {
        const workflow = classifyEventWorkflow(event);
        return workflow.domain === EventWorkflowDomain.STRONG_AUTH
          && workflow.state === EventWorkflowState.STRONG_AUTH_READY;
      })
    : targetEvents;
  if (
...
export function correlateEventWorkflow(event = {}, expectedResource = {}) {
  ...
  for (const group of groups) {
    ...
    const includeResourceId = workflow.domain !== EventWorkflowDomain.PAYMENT
      || group.includes('orderId');
    const eventValues = eventGroupValues(event, group, includeResourceId);
