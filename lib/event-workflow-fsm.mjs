...
function eventTypesMatch(actual, expected) {
  const canonicalActual = canonicalAccountEventType(actual);
  const canonicalExpected = canonicalAccountEventType(expected);
  if (canonicalActual || canonicalExpected) return canonicalActual === canonicalExpected;
...
export function classifyEventPollObservation(observation = {}, waitSpec = {}) {
  ...
  if (
    ...
    const candidateResults = targetEvents.map((event) => classifyAgentPayAccountEventCandidate({
      event,
