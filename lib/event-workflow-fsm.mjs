...
function correlationGroupsFor(event = {}, workflow = {}, expectedResource = {}) {
  ...
  if (workflow.domain === EventWorkflowDomain.SKILL_TIP_ACCOUNT) {
    ...
    if (hasCustomer && hasMerchant) return [['customerId'], ['merchantId']];
    if (hasCustomer && hasSkill) return [['customerId'], ['skillId']];
    if (hasMerchant && hasSkill) return [['merchantId'], ['skillId']];
    return [];
  }
  return [];
}
