...
function screenedTextFrom(input = {}, mandates = []) {
  const texts = [];
  let invalidField = null;
  const push = (value, field) => {
    if (value === undefined || value === null || value === '') return;
...
export function classifyInstructionRestriction(input = {}) {
  ...
  const mandatesResult = mandatesFrom(input);
  ...
  const textResult = screenedTextFrom(input, mandatesResult.mandates);
