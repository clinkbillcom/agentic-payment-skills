...
function validIdentitySegment(value) {
...
function installCommand(target) {
  ...
  return packageOperand.startsWith('-')
    ? `clink-cli skills install --format json -- ${packageOperand}`
    : `clink-cli skills install ${packageOperand} --format json`;
}
...
function normalizedTarget(target = {}, requireSkillId = false) {
  ...
  if (!validIdentitySegment(publisher) || !validIdentitySegment(skillName) || !validVersion(versionNo)) {
    return null;
  }
...
function validExpectedInstall(expectedInstall) {
  ...
  return typeof publisher === 'string'
    ...
    && validIdentitySegment(publisher)
    && validIdentitySegment(skillName)
