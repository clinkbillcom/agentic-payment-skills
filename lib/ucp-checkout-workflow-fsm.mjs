...
import { INTERNAL_UCP_CATALOG_SOURCE } from './catalog-discovery-fsm.mjs';
import {
  UcpCheckoutRunAction,
  classifyUcpCheckoutRunRequest,
} from './ucp-checkout-run-fsm.mjs';
...
export const UcpCheckoutWorkflowState = Object.freeze({
  ...
  AUTHORIZATION_PREPARE_REQUIRED: 'AUTHORIZATION_PREPARE_REQUIRED',
  ...
  PURCHASE_AUTHORIZATION_REQUIRED: 'PURCHASE_AUTHORIZATION_REQUIRED',
  CHECKOUT_EXECUTION_CLAIM_REQUIRED: 'CHECKOUT_EXECUTION_CLAIM_REQUIRED',
...
export const UcpCheckoutWorkflowAction = Object.freeze({
  ...
  START_AUTHORIZATION_PREPARE_AND_WAIT: 'START_AUTHORIZATION_PREPARE_AND_WAIT',
  ...
  CLAIM_UCP_CHECKOUT_ATTEMPT: 'CLAIM_UCP_CHECKOUT_ATTEMPT',
...
function paymentSuccessPollCommand(checkoutId, ucpOrderId, checkoutEndpoint, nextToken) {
  return [
    'clink events poll --type agent_order.succeeded',
    checkoutId ? `--checkout-id ${shellQuoteIfNeeded(checkoutId)}` : null,
    ucpOrderId ? `--ucp-order-id ${shellQuoteIfNeeded(ucpOrderId)}` : null,
    checkoutEndpoint ? `--endpoint ${shellQuoteIfNeeded(checkoutEndpoint)}` : null,
    nextToken ? `--next-token ${shellQuoteIfNeeded(nextToken)}` : null,
...
const CATALOG_ENVIRONMENT_ORIGINS = new Map([
...
function checkoutAttemptIdResolution(input = {}) {
  const aliases = ['checkoutAttemptId', 'checkout_attempt_id', 'attemptId', 'attempt_id'];
  const supplied = aliases
    .filter((alias) => hasOwn(input, alias))
    .map((alias) => input[alias]);
  if (supplied.length === 0) return { valid: false, value: null };
  ...
  if (normalized.some((value) => value === null) || new Set(normalized).size !== 1) {
    return { valid: false, value: null };
  }
  return { valid: true, value: normalized[0] };
}

function consistentCatalogEnvironment(values) {
  const environments = values
    .map((value) => normalizedString(value)?.toLowerCase() ?? null)
    .filter((value) => value !== null);
  return {
    valid: new Set(environments).size <= 1,
    value: environments[0] ?? null,
  };
}

function consistentWalletOrigin(values) {
  const baseUrls = values
    .map((value) => normalizedString(value))
    .filter((value) => value !== null);
  const origins = baseUrls.map((baseUrl) => {
    try {
  ...
  return {
    valid: origins.every((origin) => origin !== null)
      && new Set(origins).size <= 1,
    value: origins[0] ?? null,
  };
}
...
function walletStatusOrigin(rawStatus) {
  const parsed = parseMaybeJson(rawStatus);
  if (
    !parsed
    || typeof parsed !== 'object'
    || Array.isArray(parsed)
    || (hasOwn(parsed, 'ok') && parsed.ok !== true)
    || (hasOwn(parsed, 'error') && hasNonEmptyError(parsed.error))
  ) {
    return { valid: false, reason: 'catalog_checkout_wallet_environment_unverified' };
  }
  let data = parsed;
  ...
  const origin = consistentWalletOrigin([data.baseUrl, data.base_url]);
  if (!origin.valid) {
    return { valid: false, reason: 'catalog_checkout_wallet_environment_conflict' };
  }
  if (!origin.value) {
    return { valid: false, reason: 'catalog_checkout_wallet_environment_unverified' };
  }
...
function catalogCheckoutEnvironmentCheck(input = {}) {
  const selectedProductValues = ['selectedProduct', 'selected_product']
    .filter((key) => hasOwn(input, key))
    .map((key) => input[key]);
  const selectedProductProvided = selectedProductValues.length > 0;
  if (
    selectedProductValues.some(
      (value) => !value || typeof value !== 'object' || Array.isArray(value),
    )
  ) {
    return {
      valid: false,
      reason: 'catalog_checkout_selected_product_context_invalid',
    };
  }
  const selectedEnvironment = consistentCatalogEnvironment(
    selectedProductValues.flatMap((selectedProduct) => [
      selectedProduct.catalogEnvironment,
      selectedProduct.catalog_environment,
    ]),
  );
  const selectedEnvironmentMissing = selectedProductValues.some((selectedProduct) => (
    !consistentCatalogEnvironment([
      selectedProduct.catalogEnvironment,
      selectedProduct.catalog_environment,
    ]).value
  ));
  const inputEnvironment = consistentCatalogEnvironment([
    input.catalogEnvironment,
    input.catalog_environment,
  ]);
  if (
    !selectedEnvironment.valid
    || !inputEnvironment.valid
    || (
      selectedEnvironment.value
      && inputEnvironment.value
      && selectedEnvironment.value !== inputEnvironment.value
    )
  ) {
    return {
      valid: false,
      reason: 'catalog_checkout_environment_context_conflict',
      ...(selectedEnvironment.value
        ? { selectedCatalogEnvironment: selectedEnvironment.value }
        : {}),
      ...(inputEnvironment.value ? { inputCatalogEnvironment: inputEnvironment.value } : {}),
    };
  }
  if (selectedProductProvided && selectedEnvironmentMissing) {
    return {
      valid: false,
      reason: 'catalog_checkout_environment_context_missing',
      missing: ['selectedProduct.catalogEnvironment'],
    };
  }

  // A selected product is the frozen Catalog result. Top-level input remains a compatibility
  // fallback only when no selected-product object was supplied.
  const catalogEnvironment = selectedEnvironment.value ?? inputEnvironment.value;
  if (!catalogEnvironment) return null;

  const expectedWalletOrigin = CATALOG_ENVIRONMENT_ORIGINS.get(catalogEnvironment) ?? null;
  if (!expectedWalletOrigin) {
    return {
      valid: false,
      reason: 'catalog_checkout_environment_invalid',
      catalogEnvironment,
    };
  }

  const walletStatusValues = ['walletStatus', 'wallet_status']
    .filter((key) => hasOwn(input, key))
    .map((key) => input[key]);
  const walletStatusProvided = walletStatusValues.length > 0;
  if (!walletStatusProvided) {
    return {
      valid: false,
      reason: 'catalog_checkout_wallet_environment_unverified',
      catalogEnvironment,
      expectedWalletOrigin,
    };
  }
  const statusOriginResolutions = walletStatusValues.map(walletStatusOrigin);
  const invalidStatusOrigin = statusOriginResolutions.find((resolution) => !resolution.valid);
  if (invalidStatusOrigin) {
    return {
      valid: false,
      reason: invalidStatusOrigin.reason,
      catalogEnvironment,
      expectedWalletOrigin,
    };
  }
  const statusOrigins = statusOriginResolutions.map((resolution) => resolution.value);
  if (new Set(statusOrigins).size > 1) {
    return {
      valid: false,
      reason: 'catalog_checkout_wallet_environment_conflict',
      catalogEnvironment,
      expectedWalletOrigin,
      walletStatusOrigins: statusOrigins,
    };
  }
  const statusWalletOrigin = statusOrigins[0] ?? null;
  const inputWalletOrigin = consistentWalletOrigin([
    input.walletBaseUrl,
    input.wallet_base_url,
  ]);
  if (
    !inputWalletOrigin.valid
    || (
      statusWalletOrigin
      && inputWalletOrigin.value
      && statusWalletOrigin !== inputWalletOrigin.value
    )
  ) {
    return {
      valid: false,
      ...
      catalogEnvironment,
      expectedWalletOrigin,
      ...(statusWalletOrigin ? { walletStatusOrigin: statusWalletOrigin } : {}),
      ...(inputWalletOrigin.value ? { inputWalletOrigin: inputWalletOrigin.value } : {}),
    };
  }
  // Current wallet status is authoritative. An explicit URL may only corroborate it or conflict.
  const walletOrigin = statusWalletOrigin;
  if (!walletOrigin) {
    return {
      valid: false,
      reason: 'catalog_checkout_wallet_environment_unverified',
  ...
  if (walletOrigin !== expectedWalletOrigin) {
    return {
      valid: false,
      reason: 'catalog_checkout_environment_mismatch',
      catalogEnvironment,
      expectedWalletOrigin,
      walletOrigin,
    };
...
export function normalizeUcpAmountToMinorUnitLong({ amount, currency, scale = 'major' } = {}) {
  ...
  if (typeof amount !== 'string') {
    throw new Error('major-unit amount must be a decimal string');
  }
...
function intentQuantity(input = {}) {
  if (input.quantity === undefined || input.quantity === null || input.quantity === '') return 1;
...
export function classifyUcpCheckoutPrerequisites(input = {}) {
  const catalogEnvironmentCheck = catalogCheckoutEnvironmentCheck(input);
  if (catalogEnvironmentCheck && !catalogEnvironmentCheck.valid) {
    return {
      state: UcpCheckoutWorkflowState.CHECKOUT_FAILED,
      action: UcpCheckoutWorkflowAction.STOP_CHECKOUT_FAILURE,
      terminal: true,
      ...catalogEnvironmentCheck,
    };
  }

  const missing = [];
  const selectedProduct = input.selectedProduct && typeof input.selectedProduct === 'object'
    && !Array.isArray(input.selectedProduct)
    ? input.selectedProduct
    : {};
  const source = normalizedString(
    input.source
      ?? input.productSource
      ?? input.product_source
      ?? selectedProduct.source
      ?? selectedProduct.productSource
      ?? selectedProduct.product_source,
  );
  const internalCatalog = source === INTERNAL_UCP_CATALOG_SOURCE;
  const productUrl = normalizedString(
    input.productUrl
      ?? input.url
  ...
  const merchantUrl = normalizedString(
    input.merchantUrl
      ?? input.merchant_url
      ?? selectedProduct.merchantUrl
      ?? selectedProduct.merchant_url,
  );
  const itemId = normalizedString(
    input.itemId
      ?? input.item_id
  ...
  for (const [field, value] of [
    ...(!internalCatalog ? [['productUrl', productUrl]] : []),
    ['merchantUrl', merchantUrl],
    ...(internalCatalog ? [['itemId', itemId]] : []),
  ...
  const quantity = input.quantity;
  if (!Number.isSafeInteger(quantity) || quantity <= 0) missing.push('quantity');
  if (normalizedString(input.fulfillmentType) === null) missing.push('fulfillmentType');

  if (missing.length > 0) {
    const needsProductExploration = !internalCatalog
      && productUrl !== null
  ...
  if (normalizedString(input.paymentInstrumentId ?? input.payment_instrument_id) === null) {
    const refreshAttempted = [
      input.paymentInstrumentRefreshAttempted,
      input.payment_instrument_refresh_attempted,
      input.paymentMethodsRefreshed,
      input.payment_methods_refreshed,
    ...
    if (refreshAttempted && unattended) {
      return {
        ...
        terminal: true,
        reason: 'unattended_payment_instrument_missing',
      };
    }
    if (refreshAttempted) {
      return {
        state: UcpCheckoutWorkflowState.AUTHORIZATION_PREPARE_REQUIRED,
        action: UcpCheckoutWorkflowAction.START_AUTHORIZATION_PREPARE_AND_WAIT,
        terminal: false,
        reason: 'no_payment_instrument_pending_instruction_required',
      };
    }
    return {
...
export function classifyUcpCheckoutRunExecution(input = {}) {
  const run = classifyUcpCheckoutRunRequest(input);
  if (run.action === UcpCheckoutRunAction.ASK_FOR_PURCHASE_CONFIRMATION) {
    return {
      state: UcpCheckoutWorkflowState.PURCHASE_AUTHORIZATION_REQUIRED,
      action: UcpCheckoutWorkflowAction.ASK_FOR_PURCHASE_AUTHORIZATION,
      terminal: false,
      reason: run.reason,
    };
  }
  if (run.action === UcpCheckoutRunAction.FIX_CHECKOUT_RUN_GATES) {
    const nextGate = run.missing[0];
    const gateAction = {
      ...
      checkoutRouteResolved: {
        state: UcpCheckoutWorkflowState.CHECKOUT_ROUTE_REQUIRED,
        action: UcpCheckoutWorkflowAction.RESOLVE_CHECKOUT_ROUTE,
      },
      checkoutExecutionClaimed: {
        state: UcpCheckoutWorkflowState.CHECKOUT_EXECUTION_CLAIM_REQUIRED,
        action: UcpCheckoutWorkflowAction.CLAIM_UCP_CHECKOUT_ATTEMPT,
      },
    }[nextGate];
    if (nextGate === 'checkoutExecutionClaimed') {
      const attempt = checkoutAttemptIdResolution(input);
      if (!attempt.valid) {
        return {
          state: UcpCheckoutWorkflowState.CLI_ERROR,
          action: UcpCheckoutWorkflowAction.SURFACE_ERROR,
          terminal: true,
          reason: 'checkout_attempt_id_invalid',
          invalid: ['checkoutAttemptId'],
        };
      }
      // This pure classifier requests a claim; it does not perform or infer the atomic transition.
      // The runtime may set checkoutExecutionClaimed=true only for the unique transition winner.
      return {
        ...gateAction,
        terminal: false,
        reason: `${nextGate}_not_ready`,
        missing: run.missing,
    ...
    return {
      ...gateAction,
      terminal: nextGate === 'restrictedCategoryGatePassed',
      reason: `${nextGate}_not_ready`,
      missing: run.missing,
    };
  }
  if (run.action !== UcpCheckoutRunAction.RUN_UCP_CHECKOUT) {
    return {
      state: UcpCheckoutWorkflowState.CLI_ERROR,
      action: UcpCheckoutWorkflowAction.SURFACE_ERROR,
      terminal: true,
  ...
  return {
    state: UcpCheckoutWorkflowState.CHECKOUT_RUN_READY,
    action: UcpCheckoutWorkflowAction.RUN_UCP_CHECKOUT,
    terminal: false,
    reason: run.reason,
    checkoutAttemptId: run.frozenRequest.checkoutAttemptId,
    runCommand: run.command,
    expectedRun: run.frozenRequest,
  };
}
...
function classifyCheckoutStatus(
  status,
  checkoutId,
  operation,
  data = {},
  { frozenUcpOrderId, checkoutEndpoint } = {},
) {
  const fields = resourceFields(data);
  if (status === 'completed' || status === 'complete' || status === 'succeeded'
      || status === 'success' || status === 'complete_in_progress') {
    return {
      state: UcpCheckoutWorkflowState.PAYMENT_SUCCESS_EVENT_REQUIRED,
      action: UcpCheckoutWorkflowAction.POLL_PAYMENT_SUCCESS_EVENT,
      terminal: false,
      reason: status === 'complete_in_progress'
        ? 'complete_in_progress_poll_payment_success_event'
        : 'completed_poll_payment_success_event',
      ...fields,
      pollCommand: paymentSuccessPollCommand(
        checkoutId,
        frozenUcpOrderId ?? fields.ucpOrderId,
        checkoutEndpoint,
      ),
    };
  }
...
export function classifyUcpPaymentSuccessEventObservation(observation = {}, expectedResource = {}) {
  ...
  const nextTokenAliases = classifyIdentifierAliases([data.nextToken, data.next_token]);
  if (!nextTokenAliases.consistent) {
    return {
      state: UcpCheckoutWorkflowState.CLI_ERROR,
      action: UcpCheckoutWorkflowAction.SURFACE_ERROR,
      terminal: true,
      paymentConfirmed: false,
      reason: 'payment_success_event_next_token_invalid',
    };
  }
  ...
  if (successEvent) {
    ...
    const eventAckWarning = typeof data.eventAckWarning === 'string'
      ? normalizedString(data.eventAckWarning) ?? undefined
      : undefined;
    ...
    // Newer CLI bundles deterministically continue the checkout-scoped success poll into the
    // authoritative UCP-order lookup in the same process. Consume that attached result here so an
    // Agent never has to schedule the intermediate FETCH/GET actions. Older bundles omit these
    // fields and continue through the compatibility path below.
    const embeddedOrderLookup = data.paymentConfirmed === true
      || data.orderLookupStatus !== undefined
      || data.ucpOrderId !== undefined
      || data.order !== undefined
      || data.orderWarning !== undefined;
    if (embeddedOrderLookup) {
      const embeddedUcpOrderAliases = contextUcpOrderIdAliasesOf(data);
      const expectedUcpOrderId = ucpOrderAliases.value;
      const embeddedUcpOrderId = embeddedUcpOrderAliases.value;
      if (!embeddedUcpOrderAliases.consistent
          || (expectedUcpOrderId && embeddedUcpOrderId
            && expectedUcpOrderId !== embeddedUcpOrderId)) {
        return {
          state: UcpCheckoutWorkflowState.ORDER_FETCH_FAILED,
          action: UcpCheckoutWorkflowAction.RETURN_PAYMENT_SUCCESS_WITH_ORDER_WARNING,
          terminal: true,
          reason: 'embedded_ucp_order_id_conflict',
          ...paymentEvidence,
          ...(expectedUcpOrderId ? { expectedUcpOrderId } : {}),
          ...(embeddedUcpOrderId ? { observedUcpOrderId: embeddedUcpOrderId } : {}),
          orderLookupStatus: 'IDENTIFIER_CONFLICT',
          warning: 'Payment succeeded, but the CLI returned conflicting UCP order identifiers.',
        };
      }

      const resolvedUcpOrderId = expectedUcpOrderId ?? embeddedUcpOrderId;
      const lookupStatus = normalizedString(data.orderLookupStatus)?.toUpperCase();
      if (lookupStatus === 'FETCHED' && resolvedUcpOrderId) {
        return classifyUcpOrderFetchObservation(
          {
            exitCode: 0,
            stdout: JSON.stringify({ ok: true, data: data.order }),
          },
          { ...paymentEvidence, ucpOrderId: resolvedUcpOrderId },
        );
      }

      const orderWarning = normalizedString(data.orderWarning)
        ?? 'Payment succeeded, but UCP order details could not be fetched.';
      const orderResumeCommand = normalizedString(data.orderResumeCommand);
      return {
        state: UcpCheckoutWorkflowState.ORDER_FETCH_FAILED,
        action: UcpCheckoutWorkflowAction.RETURN_PAYMENT_SUCCESS_WITH_ORDER_WARNING,
        terminal: true,
        reason: lookupStatus
          ? `embedded_ucp_order_lookup_${lookupStatus.toLowerCase()}`
          : 'embedded_ucp_order_lookup_invalid',
        ...paymentEvidence,
        ...(resolvedUcpOrderId
          ? { ucpOrderId: resolvedUcpOrderId, omsOrderId: resolvedUcpOrderId }
          : {}),
        orderLookupStatus: lookupStatus ?? 'ERROR',
        warning: orderWarning,
        ...(orderResumeCommand ? { orderResumeCommand } : {}),
      };
    }
  ...
  if (data.timedOut === true || data.ready === false) {
    ...
    return {
      ...
      ...(checkoutId ? {
        resumeCommand: paymentSuccessPollCommand(
          checkoutId,
          ucpOrderAliases.value,
          checkoutEndpoint,
          nextTokenAliases.value,
        ),
      } : {}),
    };
  }
    ...
    ...(expectedCheckoutAliases.value
      ? {
        resumeCommand: paymentSuccessPollCommand(
          expectedCheckoutAliases.value,
          ucpOrderAliases.value,
          checkoutEndpointOf(expectedResource),
          nextTokenAliases.value,
        ),
      }
      : {}),
...
export function classifyUcpOrderResolutionObservation(observation = {}, context = {}) {
  ...
  const eventAckWarning = typeof context.eventAckWarning === 'string'
    ? normalizedString(context.eventAckWarning) ?? undefined
    : undefined;
