...
function pendingTipReplyDecision(input = {}, text = '') {
  const pendingTipConfirmation = activePendingTipConfirmation(input);
  if (!pendingTipConfirmation) return null;
  const source = String(text).trim();
  if (!source || isSkillTipNonAuthorizingQuestion(source) || isSkillTipHistoricalOrConditional(source)) {
    return null;
  }
  if (/^(?:取消|不要|不用|算了|否|不确认|cancel|no|stop)(?:了|吧)?[。.!！]?$/iu.test(source)) {
    return { confirmation: 'CANCELLED', pendingTipConfirmation };
  }
  if (/^(?:确认|是的|可以|继续|确认打赏|打赏吧|yes|confirm|proceed)(?:了|吧)?[。.!！]?$/iu.test(source)) {
    return { confirmation: 'CONFIRMED', pendingTipConfirmation };
  }
  return null;
}
...
export function classifyPaymentIntent(input = {}) {
  ...
  const pendingReply = pendingTipReplyDecision(input, text);
  if (pendingReply?.confirmation === 'CONFIRMED') {
    return {
  ...
  if (pendingReply?.confirmation === 'CANCELLED') {
    return {
