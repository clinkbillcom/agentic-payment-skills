...
function activePendingCatalogProductSelection(input = {}) {
  const pending = input.pendingCatalogProductSelection ?? input.pending_catalog_product_selection;
  return pending && typeof pending === 'object' && !Array.isArray(pending)
...
function pendingCatalogCandidates(pending = {}) {
  const candidates = pending.candidates ?? pending.products;
...
function catalogSelectionRestartDecision(pending = {}, context = {}) {
  const catalogQuery = firstNormalizedString(
    pending.catalogQuery,
    pending.catalog_query,
    pending.query,
  );
  return {
...
function catalogCandidateIdentity(candidate = {}, pending = {}) {
  const context = catalogSelectionContext(candidate, pending);
  return {
    productId: firstNormalizedString(candidate.productId, candidate.product_id, candidate.id),
    productUrl: firstNormalizedString(candidate.productUrl, candidate.product_url, candidate.url),
    productName: firstNormalizedString(candidate.productName, candidate.product_name, candidate.title),
    merchantId: firstNormalizedString(candidate.merchantId, candidate.merchant_id),
    storeId: firstNormalizedString(candidate.storeId, candidate.store_id),
    channelType: firstNormalizedString(candidate.channelType, candidate.channel_type),
    region: firstNormalizedString(candidate.region),
    catalogEnvironment: context.valid ? context.catalogEnvironment : null,
    catalogLanguage: context.valid ? context.catalogLanguage : null,
  };
}
...
function replyOrdinal(text = '') {
  const source = String(text).trim();
  const digits = source.match(/^(?:第\s*)?(\d{1,2})\s*(?:个|号|项|件)?[。.!！]?$/u)?.[1];
  if (digits) return Number(digits);
  const chinese = source.match(/^第?\s*([一二两三四五六七八九十])\s*(?:个|号|项|件)?[。.!！]?$/u)?.[1];
  return chinese ? CHINESE_ORDINALS.get(chinese) ?? null : null;
}

function selectedCatalogCandidate(input = {}, text = '', candidates = []) {
  const explicitProductId = firstNormalizedString(
  ...
  if (explicitProductId) {
    const matched = candidates.find(
      (candidate) => catalogCandidateIdentity(candidate).productId === explicitProductId,
    );
    return matched
      ? { candidate: matched }
      : { unresolved: 'selected_product_not_in_candidates', rejectedProductId: explicitProductId };
  }

  const explicitIndex = input.selectedIndex ?? input.selected_index;
  if (explicitIndex !== undefined && explicitIndex !== null && explicitIndex !== '') {
    const index = Number(explicitIndex);
    if (!Number.isInteger(index) || index < 1 || index > candidates.length) {
      return { unresolved: 'selected_index_out_of_range', rejectedIndex: explicitIndex };
    }
    return { candidate: candidates[index - 1] };
  }
  ...
  if (ordinal !== null) {
    ...
    return { candidate: candidates[ordinal - 1] };
  }
...
function pendingCatalogSelectionReplyDecision(input = {}, text = '') {
  const pending = activePendingCatalogProductSelection(input);
  if (!pending) return null;

  const source = String(text).trim();
  const candidates = pendingCatalogCandidates(pending);

  const cancellation = /^(?:取消|不买了?|不用了?|算了|都不要|一个都不要|cancel|stop|none|no)(?:了|吧)?[。.!！]?$/iu.test(source);
  if (cancellation) {
    return { confirmation: 'CANCELLED', pendingCatalogProductSelection: pending };
  }
  ...
  if (!frozenContext.valid) {
    return catalogSelectionRestartDecision(pending, frozenContext);
  }
  ...
  if (selection.candidate) {
    const context = catalogSelectionContext(selection.candidate, pending, frozenContext);
    if (!context.valid) {
      return catalogSelectionRestartDecision(pending, context);
    }
    return {
      confirmation: 'SELECTED',
      pendingCatalogProductSelection: pending,
      selectedProduct: catalogCandidateIdentity(selection.candidate, pending),
    };
  }
  if (selection.unresolved) {
    return {
      ...
      pendingCatalogProductSelection: pending,
      ...(selection.rejectedProductId ? { rejectedProductId: selection.rejectedProductId } : {}),
      ...(selection.rejectedIndex !== undefined ? { rejectedIndex: selection.rejectedIndex } : {}),
    };
  }
  return {
...
const PURCHASE_VERB_PATTERN = /购买|想买|要买|买|下单|订购|\b(?:buy|purchase|order|checkout|get\s+me)\b/giu;
...
function describedProductQuery(text = '') {
  const residual = String(text)
    ...
    .replace(/\b(?:me|for|a|an|the|this|that|some|please)\b/giu, ' ')
    .trim();
  return residual.length > 0 ? String(text).trim() : null;
}
