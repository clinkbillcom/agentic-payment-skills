...
function uniqueTextIdentityTargets(text = '') {
  const targets = [...String(text).matchAll(/([A-Za-z0-9._+-]+)\/([A-Za-z0-9._+-]+)/gu)]
    .map((match) => ({ kind: 'identity', publisher: match[1], skillName: match[2] }));
  return uniqueBy(targets, (target) => `${target.publisher.toLowerCase()}\u0000${target.skillName.toLowerCase()}`);
}
