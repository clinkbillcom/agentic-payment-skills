...
function catalogCandidateIdentityResolution(candidate = {}, pending = {}, frozenContext = null) {
  ...
  const fields = {
    ...
    merchantUrl: catalogCandidateStringResolution(
      candidate,
      ['merchantUrl', 'merchant_url'],
      'merchantUrl',
    ),
    merchantDomain: catalogCandidateStringResolution(
      candidate,
      ['merchantDomain', 'merchant_domain', 'domainName', 'domain_name'],
      'merchantDomain',
    ),
    source: catalogCandidateStringResolution(
      candidate,
      ['source', 'productSource', 'product_source'],
      'source',
    ),
  ...
  if (fields.productUrl.value && !isAbsoluteHttpUrl(fields.productUrl.value)) {
    invalidFields.push('productUrl');
  }
  if (fields.merchantUrl.value && !isAbsoluteHttpUrl(fields.merchantUrl.value)) {
    invalidFields.push('merchantUrl');
  }

  const merchantId = fields.merchantId.value;
  const storeId = fields.storeId.value;
  const internalCatalog = fields.source.value === INTERNAL_UCP_CATALOG_SOURCE;
  const internalMerchantUrl = fields.merchantUrl.value;
  if (fields.source.value && !internalCatalog) invalidFields.push('source');
  ...
  if (storeId && !fields.productUrl.value) missing.push('productUrl');
  if (merchantId && !internalCatalog && !fields.productUrl.value) missing.push('productUrl');
  if (internalCatalog) {
    if (!merchantId) missing.push('merchantId');
    if (!internalMerchantUrl) missing.push('merchantUrl');
    if (!fields.merchantDomain.value) missing.push('merchantDomain');
    if (
      internalMerchantUrl
      && fields.merchantDomain.value
      && httpUrlDomain(internalMerchantUrl) !== fields.merchantDomain.value
        .toLowerCase()
        .replace(/\.+$/u, '')
    ) {
      invalidFields.push('merchantUrl');
  ...
  return {
    ...
    identity: {
      ...
      ...(fields.source.value ? { source: fields.source.value } : {}),
      ...(merchantId ? { merchantId } : {}),
      ...(fields.merchantUrl.value ? { merchantUrl: fields.merchantUrl.value } : {}),
      ...(fields.merchantDomain.value ? { merchantDomain: fields.merchantDomain.value } : {}),
      ...(storeId ? { storeId } : {}),
...
function internalCatalogCheckoutProvenance(input, target) {
  const pending = input.pendingCatalogProductSelection
    ?? input.pending_catalog_product_selection;
  const selectedProduct = input.selectedProduct ?? input.selected_product;
  if (!isRecord(pending) || pending.status !== 'EXECUTING' || !isRecord(selectedProduct)) {
    return {
      valid: false,
      reason: 'routing_contract_internal_catalog_provenance_missing',
      missing: ['pendingCatalogProductSelection', 'selectedProduct'],
    };
  }

  const provenance = catalogSelectionProvenance(pending);
  const frozenContext = frozenCatalogSelectionContext(pending);
  const query = pendingCatalogQueryResolution(pending);
  const candidates = pendingCatalogCandidatesResolution(pending);
  if (
    !provenance.valid
    || provenance.mode !== 'PURCHASE_SELECTION'
    || !frozenContext.valid
    ...
    return {
      valid: false,
      reason: 'routing_contract_internal_catalog_provenance_invalid',
    };
  }

  const selectedId = [...new Set(catalogCandidateProductIdValues(selectedProduct))];
  ...
  const matchingCandidates = candidates.candidates.filter(
    (candidate) => catalogCandidateProductIdValues(candidate).includes(selectedId[0]),
  );
  ...
  const candidate = catalogCandidateIdentityResolution(
  ...
  const expected = {
    ...
    itemId: identity.productId,
    productName: identity.productName,
    catalogEnvironment: identity.catalogEnvironment,
    catalogLanguage: identity.catalogLanguage,
  };
  const selectedExpected = {
    source: selectedProduct.source,
    merchantId: selectedProduct.merchantId,
    merchantUrl: selectedProduct.merchantUrl,
    merchantDomain: selectedProduct.merchantDomain,
    itemId: selectedProduct.productId,
    productName: selectedProduct.productName,
    catalogEnvironment: selectedProduct.catalogEnvironment,
    catalogLanguage: selectedProduct.catalogLanguage,
  };
  const fields = Object.keys(expected);
  if (
    fields.some((field) => expected[field] !== selectedExpected[field])
    || fields.some((field) => expected[field] !== target[field])
  ) {
    return {
      valid: false,
      reason: 'routing_contract_internal_catalog_provenance_conflict',
      invalidFields: fields
        .filter((field) => (
          expected[field] !== selectedExpected[field]
          || expected[field] !== target[field]
        ))
        .map((field) => `target.${field}`),
    };
  }
  return { valid: true, selectedProduct: identity };
}
...
export function classifyPaymentIntentV2(input = {}) {
  ...
  if (operation === PaymentRoutingOperation.UCP_CHECKOUT) {
    ...
    const productName = optionalContractString(target, 'productName');
    const merchantId = optionalContractString(target, 'merchantId');
    const merchantUrl = optionalContractString(target, 'merchantUrl');
    const merchantDomain = optionalContractString(target, 'merchantDomain');
    const source = optionalContractString(target, 'source');
    const internalCatalog = source.value === INTERNAL_UCP_CATALOG_SOURCE;
    const invalidFields = [
      ...
      ...(!merchantUrl.valid ? ['target.merchantUrl'] : []),
      ...(!merchantDomain.valid ? ['target.merchantDomain'] : []),
      ...(!source.valid ? ['target.source'] : []),
      ...(productUrl.value && !isAbsoluteHttpUrl(productUrl.value) ? ['target.productUrl'] : []),
      ...(merchantUrl.value && !isAbsoluteHttpUrl(merchantUrl.value)
        ? ['target.merchantUrl']
        : []),
      ...(merchantUrl.value
        && merchantDomain.value
        && httpUrlDomain(merchantUrl.value) !== merchantDomain.value.toLowerCase().replace(/\.+$/u, '')
        ? ['target.merchantUrl']
        : []),
      ...(source.value && !internalCatalog ? ['target.source'] : []),
    ];
    if (invalidFields.length > 0) {
    ...
    if (internalCatalog) {
      const catalogContext = v2CatalogContext(input, operation, target);
      if (catalogContext.workflow) return catalogContext.workflow;
      const missing = [
        ...(!merchantId.value ? ['target.merchantId'] : []),
        ...(!merchantUrl.value ? ['target.merchantUrl'] : []),
        ...(!merchantDomain.value ? ['target.merchantDomain'] : []),
        ...(!itemId.value ? ['target.itemId'] : []),
        ...(!productName.value ? ['target.productName'] : []),
      ];
      if (missing.length > 0) {
        return routingContractInputRequired(input, operation, missing, {
          reason: 'routing_contract_internal_catalog_target_missing',
        });
      }
      const provenance = internalCatalogCheckoutProvenance(input, target);
      if (!provenance.valid) {
        return routingContractInputRequired(
          input,
          operation,
          provenance.missing ?? provenance.invalidFields ?? ['selectedProduct'],
          {
            reason: provenance.reason,
            ...(provenance.invalidFields ? { invalidFields: provenance.invalidFields } : {}),
          },
        );
      }
      return {
        ...
        terminal: false,
        reason: 'structured_internal_catalog_checkout_intent',
        operation,
        executionDecision,
        authorizationSource: input.authorizationSource,
        ...
        merchantDomain: merchantDomain.value,
        itemId: itemId.value,
        productName: productName.value,
        catalogEnvironment: catalogContext.catalogEnvironment,
        catalogLanguage: catalogContext.catalogLanguage,
        selectedProduct: provenance.selectedProduct,
        ...(productUrl.value ? { productUrl: productUrl.value } : {}),
        requiresProductParse: false,
        validateItemAgainstProductUrl: false,
        purchaseIntent: true,
        requiresWallet: true,
        authenticationMode: 'AUTHENTICATED',
        walletGate: PaymentWalletGate.REQUIRE_STATUS,
